...
/// <summary>
///     Tests for <see cref="Renderer" />, covering all rendering modes,
///     register combinations, and dirty-skip / partial-rendering optimizations.
/// </summary>
public class RendererTests {
    ...
    public void RenderEgaGraphics_CombinesPlanesTo4BitIndex() {
        ...
        // All planes set bit 7 → index = 0b1111 = 15
    ...
    public void RenderEgaGraphics_EachBitProducesOnePixel() {
        ...
        // Only plane 0 has bit 7 set (others 0) → index = 0b0001 = 1 for first pixel
        // Only plane 0 has bit 6 set → index = 1 for second pixel
    ...
    public void RenderEgaGraphics_AllPlanesContribute() {
        ...
        // Plane 0 bit 7, plane 1 bit 7, plane 2 bit 7, plane 3 clear → index = 0b0111 = 7
    ...
    public void RenderEgaGraphics_UsesAttributeMap() {
        ...
        // Index 5 = 0b0101 → plane0 bit set, plane2 bit set, others clear
        vram.Planes[0, 0] = 0x80;
        vram.Planes[1, 0] = 0x00;
        vram.Planes[2, 0] = 0x80;
        vram.Planes[3, 0] = 0x00;
    ...
    public void RenderCgaGraphics_InterleavesPlanes02And13() {
        ...
        // For bits 7-6: plane0 = 0b11xxxxxx, plane2 = 0b01xxxxxx →
    ...
    public void RenderTextMode_ForegroundAndBackgroundMixed() {
        ...
        vram.Planes[0, 0] = charCode;
        vram.Planes[1, 0] = attribute;
        // Font byte = 0b10101010 → alternating foreground/background
        ...
        frame[0].Should().Be(fg, "bit 7 set → foreground");
        frame[1].Should().Be(bg, "bit 6 clear → background");
        frame[2].Should().Be(fg, "bit 5 set → foreground");
        frame[3].Should().Be(bg, "bit 4 clear → background");
    }
    ...
    public void RenderTextMode_BlinkHighShowsNormal() {
        ...
        // Blink phase high → normal display
        blinkState.IsBlinkPhaseHigh = true;
        blinkState.MarkChanged();
        uint[] frame = RenderAndCapture(renderer, state);

        // No swap → foreground = 2
    ...
    public void BeginFrame_NoDirty_SkipsRendering() {
        ...
        // Second frame: nothing changed → skipped, no pending frame
        RenderFullFrame(renderer, state);
    ...
    public void MidFrameDirty_ResumesRenderingFromDirtyPoint() {
        ...
        // Begin frame (clean) → _skipThisFrame = true
        renderer.BeginFrame();
    ...
    public void CharacterClockMask_CountByFour_IncrementsEvery4thChar() {
        ...
        // char 0 reads addr 0 then increments → addr becomes 1
        // chars 1-3 read addr 1 (no increment since (charCounter & 3) != 0)
        // char 4 reads addr 1 then increments → addr becomes 2
        ...
        // Char 0 reads address 0 → plane0=0x80, bit 7 = 1 → index 1
        frame[0].Should().Be(state.DacRegisters.AttributeMap[1], "char 0 reads address 0");
        // Char 1 reads address 1 (after char 0 incremented) → plane0=0x40, bit 7 = 0
        frame[8].Should().Be(state.DacRegisters.AttributeMap[0], "char 1 reads address 1, bit 7 = 0");
        // Char 1, pixel 1 = bit 6 of addr 1 = 1 → index 1
    ...
    public void CharacterClockMask_CountByTwo_IncrementsEveryOtherChar() {
        ...
        // char 2 reads addr 1, then increments → addr becomes 2
        vram.Planes[0, 0] = 0x80; // bit 7 set at address 0
        ...
        frame[0].Should().Be(state.DacRegisters.AttributeMap[1], "char 0 reads address 0");
        // Char 1 reads address 1 (after char 0 incremented) → all zeros → index 0
    ...
    public void ScanlineBit0ForAddressBit13_SubstitutesOnOddScanlines() {
        // 2 lines tall with MaximumScanline=1 → char row has scanlines 0 and 1
        ...
        // On scanline 0: bit13 cleared → address stays low
        // On scanline 1: bit13 set → address gets 0x2000 OR'd in
    ...
    public void LineCompare_ResetsMemoryAddressAtCompareValue() {
        ...
        int width = renderer.Width;
        // Line 0: reads from address 0, bit 7 = 1 → index 1
    ...
    public void ColorPlaneEnable_DisabledPlaneOutputsZero() {
        ...
        frame[0].Should().Be(state.DacRegisters.AttributeMap[2], "plane 0 disabled → contributes 0");
    }
    ...
    public void ColorPlaneEnable_AllDisabled_AllZero() {
        ...
        frame[0].Should().Be(state.DacRegisters.AttributeMap[0], "all planes disabled → index 0");
    }
    ...
    public void TextMode_9DotsPerClock_Outputs9Pixels() {
        ...
        // First 8 pixels should be foreground (font bits), 9th too (mask 0x80>>8 = 0 → background)
        ...
        // So pixel 8 (9th dot) has mask=0 → always background color
        for (int i = 0; i < 8; i++) {
    ...
    public void TextMode_ExtendedMemory_UsesCharacterMapA() {
        ...
        byte charCode = 1;
        byte attribute = 0x0F; // bit 3 set → use CharacterMapA, fg = 0x7 (bits 0-2)
    ...
    public void BeginFrame_ZeroSize_DeactivatesFrame() {
        VideoState state = ConfigureBase(ClockSelect.Use25175Khz, halfDot: true);
        // VerticalDisplayEnd = 0 with ScanDouble = true → Height = (0+1)/2 = 0
    ...
    /// </summary>
    private static (Renderer renderer, VideoMemory videoMemory) CreateRenderer(VideoState state) {
        return CreateRenderer(state, new VgaBlinkState());
    }

    private static (Renderer renderer, VideoMemory videoMemory) CreateRenderer(VideoState state, VgaBlinkState blinkState) {
        ...
        Renderer renderer = new(mockMemory, state, blinkState, loggerService);
    ...
    /// <summary>
    ///     Configure for text mode (80×lines), given number of visible lines.
    /// </summary>
