...
public class BreakpointTests {
    ...
    public void TestMemoryBreakpoints(bool enableCfgCpu) {
        RunWithTimeout(() => {
            using Spice86DependencyInjection spice86DependencyInjection =
                new Spice86Creator("add", enableCfgCpu).Create();
            ...
            // simple read
            // 2 reads, but the breakpoint is removed after the first
            ...
            var readWrite0 =
                new AddressBreakPoint(BreakPointType.MEMORY_ACCESS, 0, _ => { readWrite0Triggered++; }, false);
    ...
    private static void RunWithTimeout(Action action, int timeoutMilliseconds = 5000) {
        var task = Task.Run(action);
        if (!task.Wait(timeoutMilliseconds)) {
            Assert.Fail($"Test exceeded {timeoutMilliseconds} ms timeout.");
        }
    }

    [AssertionMethod]
    private void AssertAddressMemoryBreakPoint(EmulatorBreakpointsManager emulatorBreakpointsManager,
        BreakPointType breakPointType, uint address, int expectedTriggers, bool isRemovedOnTrigger, Action action) {
        int count = 0;
        var breakPoint = new AddressBreakPoint(breakPointType, address, _ => { count++; }, isRemovedOnTrigger);
    ...
    public void TestExecutionBreakpoints(bool enableCfgCpu) {
        RunWithTimeout(() => {
            ...
            uint entryAddress = state.IpPhysicalAddress;
            emulatorBreakpointsManager.ToggleBreakPoint(new AddressBreakPoint(BreakPointType.CPU_EXECUTION_ADDRESS,
                    entryAddress, _ => { executionTriggered = true; }, true),
                true);
            emulatorBreakpointsManager.ToggleBreakPoint(
                new AddressBreakPoint(BreakPointType.MACHINE_STOP, 0, _ => { stopTriggered = true; }, true),
                true);
            programExecutor.Run();
            Assert.True(stopTriggered, "Machine stop breakpoint did not trigger.");
            Assert.True(executionTriggered, "Execution breakpoint did not trigger.");
        });
    }
    ...
    public void TestIoBreakpoints(bool enableCfgCpu) {
        RunWithTimeout(() => {
            using Spice86DependencyInjection spice86DependencyInjection =
                new Spice86Creator("externalint", enableCfgCpu, maxCycles: 0xFFFFFFF, enablePit: true)
                    .Create();
            State state = spice86DependencyInjection.Machine.CpuState;
            EmulatorBreakpointsManager emulatorBreakpointsManager =
                spice86DependencyInjection.Machine.EmulatorBreakpointsManager;
            ...
            ProgramExecutor programExecutor = spice86DependencyInjection.ProgramExecutor;
            int triggers = 0;
            emulatorBreakpointsManager.ToggleBreakPoint(new AddressBreakPoint(BreakPointType.IO_WRITE, 0x43,
                _ => {
    ...
    public void TestExternalInterruptBreakpoints(bool enableCfgCpu) {
        RunWithTimeout(() => {
            ...
            ProgramExecutor programExecutor = spice86DependencyInjection.ProgramExecutor;
            int triggers = 0;
            emulatorBreakpointsManager.ToggleBreakPoint(new AddressBreakPoint(BreakPointType.CPU_INTERRUPT, 0x8,
                _ => { triggers++; }, false), true);
            programExecutor.Run();
    ...
    public void TestProgrammaticInterruptBreakpoints(bool enableCfgCpu) {
        RunWithTimeout(() => {
            using Spice86DependencyInjection spice86DependencyInjection =
                new Spice86Creator("interrupt", enableCfgCpu).Create();
            EmulatorBreakpointsManager emulatorBreakpointsManager =
                spice86DependencyInjection.Machine.EmulatorBreakpointsManager;
            ProgramExecutor programExecutor = spice86DependencyInjection.ProgramExecutor;
            int intDtriggers = 0;
            int intOtriggers = 0;
            emulatorBreakpointsManager.ToggleBreakPoint(new AddressBreakPoint(BreakPointType.CPU_INTERRUPT, 0xD,
                _ => { intDtriggers++; }, false), true);
            // Int on overflow
            emulatorBreakpointsManager.ToggleBreakPoint(new AddressBreakPoint(BreakPointType.CPU_INTERRUPT, 0x4,
                _ => { intOtriggers++; }, false), true);
            programExecutor.Run();
