...
public sealed class HttpApiServerTests {
    ...
    [Fact]
    public async Task GetByte_WhenAlreadyPaused_DoesNotUnpause() {
        SeedMemory();
        _fixture.PauseHandler.ClearReceivedCalls();
        ...
        _fixture.PauseHandler.DidNotReceive().Resume();
    }
    ...
    [Fact]
    public async Task PutByte_WhenNotPaused_PausesThenUnpauses() {
        SeedMemory();
        _fixture.PauseHandler.ClearReceivedCalls();
        _fixture.PauseHandler.IsPaused.Returns(false);
        HttpApiWriteByteRequest request = new() {
