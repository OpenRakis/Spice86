...
public class SignedSampleConversionTests {
    ...
    public void SignedByte_NegativeOne_MapsToCorrectLookupIndex() {
        ...
        // Should map to U8To16[127] = -256, NOT U8To16[255] = 32767
        
        byte byteValue = 0xFF;
        sbyte signedValue = unchecked((sbyte)byteValue);
        signedValue.Should().Be(-1, "0xFF as signed byte is -1");
        
        float convertedValue = LookupTables.ToSigned8(signedValue);
        convertedValue.Should().Be(-256.0f, "ToSigned8(-1) should be -256");
        
        // Verify the underlying index
        byte lookupIndex = (byte)(signedValue + 128);
        lookupIndex.Should().Be(127, "signed -1 + 128 = 127");
        LookupTables.U8To16[lookupIndex].Should().Be(-256.0f);
        
        // WRONG behavior (the bug we fixed):
        // float wrongValue = LookupTables.U8To16[byteValue]; // U8To16[255] = 32767
    ...
    public void SignedByte_PositiveMax_MapsToCorrectLookupIndex() {
        ...
        // Should map to U8To16[255] = 32767
    ...
    public void SignedByte_NegativeMax_MapsToCorrectLookupIndex() {
        ...
        float convertedValue = LookupTables.ToSigned8(signedValue);
        convertedValue.Should().Be(-32768.0f, "ToSigned8(-128) should be -32768");
        
        // Verify the underlying index
        byte lookupIndex = (byte)(signedValue + 128);
        lookupIndex.Should().Be(0, "signed -128 + 128 = 0");
        LookupTables.U8To16[lookupIndex].Should().Be(-32768.0f);
    }
    
    [Fact]
    public void SignedByte_Zero_MapsToCorrectLookupIndex() {
        // Byte 0x00 represents 0 as signed int8
        // Should map to U8To16[128] = 0
        ...
        float convertedValue = LookupTables.ToSigned8(signedValue);
        convertedValue.Should().Be(0.0f, "ToSigned8(0) should be 0");
        
        // Verify the underlying index
        byte lookupIndex = (byte)(signedValue + 128);
        lookupIndex.Should().Be(128, "signed 0 + 128 = 128");
        LookupTables.U8To16[lookupIndex].Should().Be(0.0f);
    }
    ...
    public void SignedConversion_MatchesDOSBoxBehavior() {
        ...
        //         return lut_u8to16[signed_buf[i] + 128];
        // C# equivalent: sbyte signedSample = unchecked((sbyte)samples[i]);
        //                value = LookupTables.ToSigned8(signedSample);
        ...
        for (int i = 0; i < testBytes.Length; i++) {
            ...
            // Verify ToSigned8 uses the correct index
            float convertedValue = LookupTables.ToSigned8(signedValue);
            convertedValue.Should().Be(LookupTables.U8To16[lookupIndex]);
        }
    }
