...
public class AstExpressionBuilderTest {
    ...
    [Fact]
    public void TestBooleanNormalization_BoolEqualsIntNonZero() {
        // Arrange: bool(true) == uint16(5) — 5 should be normalized to true
        ConstantNode boolTrue = new ConstantNode(DataType.BOOL, 1);
        ConstantNode intNonZero = new ConstantNode(DataType.UINT16, 5);
        BinaryOperationNode equalNode = new BinaryOperationNode(DataType.BOOL, boolTrue, BinaryOperation.EQUAL, intNonZero);

        // Act & Assert
        Assert.True(ToBool(equalNode));
    }

    [Fact]
    public void TestBooleanNormalization_BoolEqualsIntZero() {
        // Arrange: bool(true) == uint16(0) — 0 maps to false
        ConstantNode boolTrue = new ConstantNode(DataType.BOOL, 1);
        ConstantNode intZero = new ConstantNode(DataType.UINT16, 0);
        BinaryOperationNode equalNode = new BinaryOperationNode(DataType.BOOL, boolTrue, BinaryOperation.EQUAL, intZero);

        // Act & Assert
        Assert.False(ToBool(equalNode));
    }
