; opl_read_delay.asm - OPL port read latency test
;
; Measures how many OPL status register reads (port 0x388) occur
; between starting Timer 1 and detecting its overflow.
;
...
;
; Assemble:  nasm -f bin -o opl_read_delay.com opl_read_delay.asm
...
.status_clear:
    ...
    ; Minimal post-start delay: 1 IO read
    mov dx, OPL_ADDR
    in al, dx

    ; === Tight read loop: count reads until overflow ===
    xor cx, cx
    mov dx, OPL_ADDR
.read_loop:
    cmp cx, TIMEOUT
