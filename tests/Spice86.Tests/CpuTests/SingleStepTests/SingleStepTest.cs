...
public class SingleStepTest {
    ...
    public SingleStepTest() {
        ...
        CpuTestInitializer testInitializer = new();
        CpuTestAsserter testAsserter = new();
        _testRunner = new CpuTestRunner(testInitializer, testAsserter);
    }
    ...
    public void TestCpu(CpuModel cpuModel, string range, int maxCycles) {
        ...
        if (stream == null) {
            Assert.Fail($"Couldn't find test resource {fileName} for {cpuModel} for range {range}");
        }
...
#pragma warning restore CS0162 // Unreachable code detected
    ...
    private List<CpuTest>? ReadCpuTests(ZipArchiveEntry entry) {
        ...
        // Deserialize as DTO list then convert to model
        var dtos = JsonSerializer.Deserialize<List<CpuTestDto>>(jsonContent);
        if (dtos == null) {
            return null;
        }

        try {
            return dtos.Select(dto => CpuTestConverter.ToModel(dto)).ToList();
