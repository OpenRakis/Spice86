...
public class RevocationListHelper {
    ...
    /// <summary>
    /// Reads and combines revocation lists from the zip archive, the per-range global file,
    /// and the eternal revocation list for the given CPU model.
    /// </summary>
    ...
    public ISet<string> ReadCombinedRevocationLists(ZipArchive archive, string cpuModelString, string range) {
        ...
        // Read from per-range global file (regenerated by test runner)
        ...
        // Read from the eternal revocation list (never regenerated, permanent record)
        string eternalFileName = $"eternal_revocation_list_{cpuModelString}.txt";
        string eternalFilePath = Path.GetFullPath(Path.Join(_readBasePath, eternalFileName));
        ISet<string> eternalRevocationList = ReadRevocationListFromFile(eternalFilePath);
        foreach (string hash in eternalRevocationList) {
            combined.Add(hash);
        }
    ...
    /// <summary>
    /// Reads the eternal revocation list for the given CPU model.
    /// </summary>
    public ISet<string> ReadEternalRevocationList(string cpuModelString) {
        string eternalFileName = $"eternal_revocation_list_{cpuModelString}.txt";
        string eternalFilePath = Path.GetFullPath(Path.Join(_readBasePath, eternalFileName));
        return ReadRevocationListFromFile(eternalFilePath);
    }
    ...
    public void WriteRevocationList(TestStatistics stats, string cpuModelString, string range) {
        ...
        foreach (var kvp in stats.OpcodeStats.OrderBy(x => x.Key)) {
            ...
            // Write error reason breakdown for failing opcodes
            if (stats.OpcodeErrorReasons.TryGetValue(opcode, out Dictionary<string, int>? reasons) && reasons.Count > 0) {
                foreach (var reason in reasons.OrderByDescending(r => r.Value)) {
                    writer.WriteLine($"#   -> {reason.Key}: {reason.Value}");
                }
            }
        }
        ...
        // Write sample errors for opcodes with failures
        bool hasSamples = false;
        foreach (var kvp in stats.OpcodeSampleErrors.OrderBy(x => x.Key)) {
            if (kvp.Value.Count == 0) {
                continue;
            }
            if (!hasSamples) {
                writer.WriteLine("# Sample errors per opcode:");
                hasSamples = true;
            }
            writer.WriteLine($"# === {kvp.Key} ===");
            foreach (string sample in kvp.Value) {
                // Indent each line of the sample error
                foreach (string trimmed in sample.Split('\n').Select(line => line.TrimEnd()).Where(t => t.Length > 0)) {
