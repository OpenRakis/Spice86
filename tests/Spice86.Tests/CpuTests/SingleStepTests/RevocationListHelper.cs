...
public class RevocationListHelper {
    ...
    public ISet<string> ReadCombinedRevocationLists(ZipArchive archive, string cpuModelString, string range) {
        ...
        string globalFilePath = Path.GetFullPath(Path.Join(_readBasePath, fileName));
    ...
    /// <returns>A set of test hashes that should be skipped, or an empty set if the file does not exist</returns>
    ...
    private string GetRevocationListPath(string cpuModelString, string range) {
        ...
        return Path.Join(_writeBasePath, fileName);
    }
}
