...
public sealed class SpeculativeCfgTest {
    ...
    /// every hardware IRQ vector with a non-zero IVT entry must be registered as a CFG generation
    /// root (an execution-context entry point) so the handler flows into code generation even when
    /// no IRQ fired during discovery. This is the mechanism that prevents the
    ...
    public void ExternalEventHandlersRegisteredAsGenerationRoots() {
        ...
        foreach (byte vectorNumber in machine.DualPic.EnumerateHardwareInterruptVectorNumbers()) {
        ...
        checkedAtLeastOne.Should().BeTrue("the emulator should install at least one hardware IRQ handler");
    }
