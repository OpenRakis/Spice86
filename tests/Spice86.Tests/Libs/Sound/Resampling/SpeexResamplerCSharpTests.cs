...
public class SpeexResamplerCSharpTests {
    ...
    /// <summary>
    /// Tests that resampling a constant DC offset preserves the offset.
    /// </summary>
    [Fact]
    public void ProcessFloat_DCOffset_IsPreserved() {
        // Arrange
        var resampler = new SpeexResamplerCSharp(channels: 1, in_rate_init: 44100, out_rate_init: 48000, quality: 8);
        ...
        result.Should().Be(0);
        for (int i = 0; i < out_len; i++) {
            output[i].Should().BeApproximately(dcOffset, 0.05f, because: "DC offset should be preserved");
        }
    }

    /// <summary>
    /// Tests memory efficiency - verifies resampler doesn't allocate excessively per call.
    /// This baseline will be compared against optimized version.
    /// </summary>
    [Fact]
    public void ProcessFloat_MemoryBaseline_EstablishAllocationRate() {
        // Arrange
        var resampler = new SpeexResamplerCSharp(channels: 1, in_rate_init: 44100, out_rate_init: 48000, quality: 8);
        float[] input = GenerateTestSignal(1000, frequency: 440f, sampleRate: 44100);
        float[] output = new float[2000];
        ...
        // Establish baseline - measure allocation pattern
        GC.Collect();
        GC.WaitForPendingFinalizers();
        long memBefore = GC.GetTotalMemory(false);
        
        // Act - run multiple iterations like audio mixer would
        for (int i = 0; i < 100; i++) {
            in_len = (uint)input.Length;
            out_len = (uint)output.Length;
            resampler.ProcessFloat(0, input.AsSpan(), 0, ref in_len, output.AsSpan(), 0, ref out_len);
        }
        ...
        // Assert - baseline measurement (will improve after optimization)
        long allocated = memAfter - memBefore;
        // This baseline should be measured and then improved upon
        // For now, just verify no catastrophic leaks (< 1MB per 100 iterations)
        allocated.Should().BeLessThan(1_000_000, because: "memory allocation should remain reasonable");
    }
