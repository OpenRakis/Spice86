...
/// Z:\AUTOEXEC.BAT generated by <see cref="Spice86.Core.Emulator.OperatingSystem.Batch.DosBatchExecutionEngine.ConfigureStartupSession"/>.
/// </summary>
public class BootstrapAutoexecIntegrationTests {
    ...
    public void Bootstrap_HostRequestedProgramWithSpacesInPath_IsProperlyQuotedInAutoexecBat() {
        WithTempFile("boot spaces", tempDir => {
            ...
            // BuildStartupProgramLine uses EscapeIfNeeded which wraps the DOS path in quotes.
    ...
    public void Bootstrap_HostRequestedBatch_UsesCallToRouteAndReturnsToBatchEngine() {
        WithTempFile("boot_bat", tempDir => {
            ...
            // The bootstrap generates: CALL C:\START.BAT / @EXIT
    ...
    public void Bootstrap_ArgumentsPassedFromHost_ArePreservedThroughAutoexecBat() {
        WithTempFile("boot_args", tempDir => {
            // Arrange: the host provides ExeArgs that flow into AUTOEXEC.BAT via BuildStartupProgramLine.
    ...
    [Fact]
    public void Bootstrap_HostRequestedExecutable_WritesDirectLaunchAndExitToAutoexecBat() {
        WithTempFile("boot_exe_autoexec", tempDir => {
            string executablePath = CreateBinaryFile(tempDir, "GAME.COM", BuildExitCodeCom(0));

            Configuration configuration = CreateShellConfiguration(tempDir, executablePath);
            configuration.ShellBootstrap = false;

            using Spice86DependencyInjection spice86 = new(configuration);
            spice86.ProgramExecutor.Run();

            bool found = spice86.Machine.Dos.DosDriveManager.TryGetMemoryDrive('Z', out MemoryDrive? zDrive);
            found.Should().BeTrue();
            zDrive.Should().NotBeNull();
            if (zDrive == null) {
                throw new InvalidOperationException("Expected Z: memory drive to be available.");
            }

            string output = Encoding.ASCII.GetString(zDrive.GetFile("AUTOEXEC.BAT"));
            output.Should().Contain("C:\\GAME.COM");
            output.Should().Contain("@EXIT");
            output.Should().NotContain("CALL C:\\GAME.COM");
        });
    }
