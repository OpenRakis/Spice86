...
public class PitModeTests {
    ...
    public PitModeTests() {
        ...
        CounterConfiguratorFactory counterConfiguratorFactory = new(configuration,
            state, new PauseHandler(loggerService), loggerService);
        DualPic dualPic = new(state, ioPortDispatcher, false, false, loggerService);
        _pit = new Timer(configuration, state, ioPortDispatcher, counterConfiguratorFactory,
    ...
    private void SimulateActivations(Pit8254Counter counter, int count) {
        // Store the original activator to restore it later
        CounterActivator originalActivator = counter.Activator;
        
        try {
            // Replace with TestCounterActivator that always returns IsActivated=true
            TestCounterActivator testActivator = new TestCounterActivator(counter.Activator.Frequency);
            SetActivator(counter, testActivator);
            ...
            // Restore the original activator
            SetActivator(counter, originalActivator);
        }
    }

    /// <summary>
    /// Uses reflection to set the activator field on the counter
    /// since it's a readonly property
    /// </summary>
    private void SetActivator(Pit8254Counter counter, CounterActivator activator) {
        typeof(Pit8254Counter)
            .GetField("_activator", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance)
            ?.SetValue(counter, activator);
    }
