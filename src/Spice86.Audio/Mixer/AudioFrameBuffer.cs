...
public sealed class AudioFrameBuffer {
    private Spice86.Audio.Sound.Common.AudioFrame[] _buffer;

    public AudioFrameBuffer(int initialCapacity) {
        ArgumentOutOfRangeException.ThrowIfNegative(initialCapacity);
        _buffer = new Spice86.Audio.Sound.Common.AudioFrame[initialCapacity];
    }

    public int Count { get; private set; }

    public Spice86.Audio.Sound.Common.AudioFrame this[int index] {
        get {
    ...
    public void Add(Spice86.Audio.Sound.Common.AudioFrame frame) {
        EnsureCapacity(Count + 1);
        _buffer[Count] = frame;
        Count++;
    }

    public void AddRange(ReadOnlySpan<Spice86.Audio.Sound.Common.AudioFrame> frames) {
        if (frames.Length == 0) {
            return;
        }
    ...
    public Span<Spice86.Audio.Sound.Common.AudioFrame> AsSpan() {
        return _buffer.AsSpan(0, Count);
    }

    public Span<Spice86.Audio.Sound.Common.AudioFrame> AsSpan(int start, int length) {
