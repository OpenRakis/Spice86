...
public class ExpressionEvaluationService {
    private readonly ExpressionValueCompiler _compiler;
    private readonly Dictionary<string, Func<long>> _cache = new();

    public ExpressionEvaluationService(State state, IMemory memory) {
        _compiler = new ExpressionValueCompiler(state, memory);
    }
    ...
    public List<FormattedTextSegment>? FormatOperandValues(Instruction instruction) {
        ...
        for (int i = 0; i < instruction.OpCount; i++) {
            ...
            if (kind == OpKind.Register) {
                ...
                if (expr != null) {
                    Func<long>? func = TryCompile(expr);
                    if (func != null) {
                        long value = func();
                        if (segments.Count > 0) {
    ...
    private static void AddSeparator(List<FormattedTextSegment> segments) {
        segments.Add(new FormattedTextSegment { Text = "  ", Kind = FormatterTextKind.Text });
    }

    private static void AddRegisterValue(List<FormattedTextSegment> segments, string name, long value, int bitWidth) {
        segments.Add(new FormattedTextSegment { Text = name, Kind = FormatterTextKind.Register });
        segments.Add(new FormattedTextSegment { Text = "=", Kind = FormatterTextKind.Punctuation });
        segments.Add(new FormattedTextSegment { Text = FormatHex(value, bitWidth), Kind = FormatterTextKind.Number });
    }

    private static void AddMemoryValue(List<FormattedTextSegment> segments, Instruction instruction, long value, int bitWidth) {
        segments.Add(new FormattedTextSegment { Text = "[", Kind = FormatterTextKind.Punctuation });

        string? segment = RegisterToExpression(instruction.MemorySegment)?.ToUpperInvariant();
        if (segment != null) {
            segments.Add(new FormattedTextSegment { Text = segment, Kind = FormatterTextKind.Register });
            segments.Add(new FormattedTextSegment { Text = ":", Kind = FormatterTextKind.Punctuation });
        }
        ...
        if (baseReg != null) {
            segments.Add(new FormattedTextSegment { Text = baseReg, Kind = FormatterTextKind.Register });
            firstPart = false;
        }
        ...
        if (indexReg != null) {
            ...
            segments.Add(new FormattedTextSegment { Text = indexReg, Kind = FormatterTextKind.Register });
            firstPart = false;
        }
    ...
    private Func<long>? TryCompile(string expression) {
        if (_cache.TryGetValue(expression, out Func<long>? cached)) {
            return cached;
        }
        try {
