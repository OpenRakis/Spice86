...
public sealed class CfgGraphNode : IEquatable<CfgGraphNode> {
    ...
    /// <summary>
    /// Syntax-highlighted tokens for rendering the full node text (header + assembly).
    /// </summary>
    public List<FormattedToken> Tokens { get; init; } = [];
    ...
    public override string ToString() => string.Join("", Tokens.ConvertAll(t => t.Text));
}
