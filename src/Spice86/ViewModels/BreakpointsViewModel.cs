...
public partial class BreakpointsViewModel : ViewModelWithErrorDialogAndMemoryBreakpoints {
    ...
    public BreakpointsViewModel(State state,
        ...
        IMemory memory,
        DebuggerDecoderService? debuggerDecoderService = null) : base(uiDispatcher, textClipboard, state, memory) {
        ...
        if (debuggerDecoderService != null) {
            KnownInterruptSuggestions = BuildInterruptSuggestions(debuggerDecoderService);
            KnownIoPortSuggestions = BuildIoPortSuggestions(debuggerDecoderService);
        } else {
            KnownInterruptSuggestions = [];
            KnownIoPortSuggestions = [];
        }
    }

    private static IReadOnlyList<KnownBreakpointSuggestion> BuildInterruptSuggestions(DebuggerDecoderService service) {
        IReadOnlyList<(byte Vector, string Description)> known = service.GetKnownInterrupts();
    ...
    private static IReadOnlyList<KnownBreakpointSuggestion> BuildIoPortSuggestions(DebuggerDecoderService service) {
        IReadOnlyList<(ushort FirstPort, ushort LastPort, string Description)> known = service.GetKnownIoPorts();
        List<KnownBreakpointSuggestion> list = new(known.Count);
        foreach ((ushort first, ushort last, string description) in known) {
            string hexValue = first == last ? $"0x{first:X4}" : $"0x{first:X4}";
    ...
    /// <summary>
    /// Autocomplete suggestions for known interrupt vectors, populated from the
    /// <see cref="DebuggerDecoderService"/> knowledge base.
    /// </summary>
    ...
    private BreakpointViewModel? CreateInterruptBreakpoint() {
        if (InterruptNumber == WildcardValue) {
            return AddWildcardBreakpoint(
                ...
                () => PauseAndReportInterrupt(uint.MaxValue),
