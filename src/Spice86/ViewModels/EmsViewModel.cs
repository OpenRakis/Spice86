...
public partial class EmsViewModel : ViewModelBase, IEmulatorObjectViewModel, IMemorySearchViewModel, IDebuggerTabContentViewModel {
    ...
    private int? FindOccurrence(byte[] source, byte[] searchBytes) {
        ...
        if (_searchDirection == SearchDirection.Forward && AddressOFoundOccurence is not null) {
            ...
            return FindForward(source, searchBytes, startIndex);
        }
        ...
        if (_searchDirection == SearchDirection.Backward) {
            ...
            return FindBackward(source, searchBytes, startIndex);
        }

        return FindForward(source, searchBytes, 0);
    }

    private static int? FindForward(byte[] source, byte[] searchBytes, int startIndex) {
        if (startIndex < 0) {
            startIndex = 0;
        }
        ...
        for (int sourceIndex = startIndex; sourceIndex <= maxStart; sourceIndex++) {
            if (MatchesAt(source, searchBytes, sourceIndex)) {
                return sourceIndex;
            }
        }
        return null;
    ...
    private static int? FindBackward(byte[] source, byte[] searchBytes, int startIndex) {
        int maxStart = source.Length - searchBytes.Length;
    ...
    private static bool MatchesAt(byte[] source, byte[] searchBytes, int sourceIndex) {
