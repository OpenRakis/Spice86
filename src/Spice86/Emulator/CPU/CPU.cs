...
public class Cpu {
    ...
    public Cpu(Machine machine, JumpHandler jumpHandler, bool debugMode) {
        ...
        _staticAddressesRecorder = new StaticAddressesRecorder(State, debugMode);
    ...
    public ushort NextUint16() {
        var res = _memory.GetUint16(InternalIpPhysicalAddress);
        _internalIp += 2;
        return res;
    }

    public byte NextUint8() {
        var res = _memory.GetUint8(InternalIpPhysicalAddress);
        _internalIp++;
        return res;
    }
    ...
    private void ExecOpcode(byte opcode) {
        ...
        switch (opcode) {
            ...
            case 0x97: {
                regIndex = opcode & RegIndexMask;
                if (IsLoggingEnabled()) { SetCurrentInstructionName($"XCHG AX,{State.Registers.GetRegName(regIndex)}"); }
            ...
            case 0xB7:
                regIndex = opcode & RegIndexMask;
                if (IsLoggingEnabled()) { SetCurrentInstructionName($"MOV {State.Registers.GetReg8Name(regIndex)} ib"); }
    ...
    public void Div8(byte v2) {
        ushort v1 = State.AX;
        byte? result = CPU.Alu.Div8(v1, v2);
        if (result == null) {
            HandleDivisionError();
            return;
        }
    ...
    public void IDiv8(byte value) {
        ...
        sbyte v2 = (sbyte)value;
        sbyte? result = CPU.Alu.Idiv8(v1, v2);
        if (result == null) {
            HandleDivisionError();
            return;
        }
    ...
    public void IDiv16(ushort value) {
        ...
        short? result = CPU.Alu.Idiv16(v1, v2);
        if (result == null) {
            HandleDivisionError();
            return;
        }
    ...
    public void Div16(ushort v2) {
        ...
        ushort? result = CPU.Alu.Div16(v1, v2);
        if (result == null) {
            HandleDivisionError();
            return;
        }
    ...
    private void HandleExternalInterrupt() {
        if (_externalInterruptVectorNumber == null || !State.InterruptFlag) {
            return;
        }
        if (IsLoggingEnabled()) {
    ...
    private void JumpNear(ushort ip) {
        if (IsLoggingEnabled()) { SetCurrentInstructionName($"JMP NEAR {ConvertUtils.ToSegmentedAddressRepresentation(State.CS, ip)}"); }
