...
// // Extract regIndex from opcode private const int REG_INDEX_MASK = 0b111;

// private const Set<int> STRING_OPCODES = new HashSet<>(Arrays.asList(0xA4, 0xA5, 0xA6, 0xA7, 0xAA,
// 0xAB, 0xAC, 0xAD, 0xAE, 0xAF, 0x6C, 0x6D, 0x6E, 0x6F));

// private Alu alu;

// private CallbackHandler callbackHandler;

// // When true will crash if an interrupt targets code at 0000:0000 private bool errorOnUninitializedInterruptHandler;

// // interrupt not generated by the code private int externalInterruptVectorNumber;
...
// // is not the same) private FunctionHandler functionHandlerInUse;

// // Value used to read parts of the instruction. CPU uses this internally and adjusts IP after //
// instruction execution is done. private int internalIp;
...
// public Cpu(Machine machine, bool debugMode) { this.machine = machine; this.memory =
// machine.getMemory(); this.state = new State(); this.alu = new Alu(state); this.stack = new
// Stack(memory, state); this.functionHandler = new FunctionHandler(machine, debugMode);
// this.functionHandlerInExternalInterrupt = new FunctionHandler(machine, debugMode);
// this.functionHandlerInUse = functionHandler; this.staticAddressesRecorder = new
// StaticAddressesRecorder(state, debugMode); this.modRM = new ModRM(machine, this); }

// private throws InvalidOperationException { internalIp = state.getIP
...
// public FunctionHandler getFunctionHandler() { return this.functionHandler; }

// public FunctionHandler getFunctionHandlerInExternalInterrupt() { return
// functionHandlerInExternalInterrupt; }

// public FunctionHandler getFunctionHandlerInUse() { return functionHandlerInUse; }

// public Stack getStack() { return stack; }

// public State getState() { return state; }

// public StaticAddressesRecorder getStaticAddressesRecorder() { return staticAddressesRecorder; }

// public bool isRunning() { return running; }

// public void setCallbackHandler(CallbackHandler callbackHandler) { this.callbackHandler =
// callbackHandler; }
...
// public void setForceLog(bool forceLog) { this.forceLog = forceLog; }

// public void setIoPortDispatcher(IOPortDispatcher ioPortDispatcher) { this.ioPortDispatcher =
// ioPortDispatcher; }

// public void setRunning(bool running) { this.running = running; }

// private void addCurrentInstructionPrefix(Supplier<String> getLog) { // Optimization, do not
// calculate the log if it is not used if (isLoggingEnabled()) {
// state.addCurrentInstructionPrefix(getLog.get()); } }

// private bool isLoggingEnabled() { if (forceLog == null) { return LOGGER.isDebugEnabled(); }
// return forceLog; }

// private void setCurrentInstructionName(Supplier<String> getLog) { // Optimization, do not
// calculate the log if it is not used if (isLoggingEnabled()) {
// state.setCurrentInstructionName(getLog.get()); } }

// private class);

// (); staticAddressesRecorder.reset(); if (isLoggingEnabled()) { stateString = state.toString();
// state.resetCurrentInstructionPrefix(); state.setCurrentInstructionName(""); }
...
// // For some instructions, zero flag is not to be checked bool checkZeroFlag =
// isStringOpUpdatingFlags(opcode); int cx = state.getCX(); while (cx != 0) { // re-set the segment
// override that may have been cleared. No need to reset ip as string // instructions don't modify
// it and are only one byte. processString(opcode); cx = cx - 1;

// if (LOGGER.isTraceEnabled()) { LOGGER.trace("REP {} Loop, CX={}, ZF={}, checkZeroFlag={},
// continueZF={}", state.getCurrentInstructionNameWithPrefix(), ConvertUtils.toHex(cx),
// state.getZeroFlag(), checkZeroFlag, continueZeroFlagValue); }
...
// if (initialAL > 0x99 || initialCF) { state.setAL(state.getAL() + 0x60); finalCarryFlag = true; }
// else { finalCarryFlag = false; }

// // Undocumented behaviour alu.updateFlags8(state.getAL());
// state.setAuxiliaryFlag(finalAuxillaryFlag); state.setCarryFlag(finalCarryFlag); } case 0x28-> {
// setCurrentInstructionName(()-> "SUB rmb rb"); modRM.read(); modRM.setRm8(alu.sub8(modRM.getRm8(),
// modRM.getR8())); } case 0x29-> { setCurrentInstructionName(()-> "SUB rmw rw"); modRM.read();
// modRM.setRm16(alu.sub16(modRM.getRm16(), modRM.getR16())); } case 0x2A-> {
// setCurrentInstructionName(()-> "SUB rb rmb"); modRM.read(); modRM.setR8(alu.sub8(modRM.getR8(),
// modRM.getRm8())); } case 0x2B-> { setCurrentInstructionName(()-> "SUB rw rmw"); modRM.read();
// modRM.setR16(alu.sub16(modRM.getR16(), modRM.getRm16())); } case 0x2C-> {
// setCurrentInstructionName(()-> "SUB AL ib"); state.setAL(alu.sub8(state.getAL(), nextUint8())); }
// case 0x2D-> { setCurrentInstructionName(()-> "SUB AX iw"); state.setAX(alu.sub16(state.getAX(),
// nextUint16())); } case 0x2E->handleInvalidOpcodeBecausePrefix(opcode); case 0x2F-> {
// setCurrentInstructionName(()-> "DAS"); int initialAL = state.getAL(); bool initialCF =
// state.getCarryFlag(); bool finalAuxillaryFlag = false; bool finalCarryFlag = false;
// state.setCarryFlag(false); if ((state.getAL() & 0x0F) > 9 || state.getAuxiliaryFlag()) {
// state.setAL(state.getAL() - 6); finalCarryFlag = state.getCarryFlag() || initialCF;
// finalAuxillaryFlag = true; } if (initialAL > 0x99 || initialCF) { state.setAL(state.getAL() -
// 0x60); finalCarryFlag = true; }

// // Undocumented behaviour alu.updateFlags8(state.getAL());
// state.setAuxiliaryFlag(finalAuxillaryFlag); state.setCarryFlag(finalCarryFlag); } case 0x30-> {
// setCurrentInstructionName(()-> "XOR rmb rb"); modRM.read(); modRM.setRm8(alu.xor8(modRM.getRm8(),
// modRM.getR8())); } case 0x31-> { setCurrentInstructionName(()-> "XOR rmw rw"); modRM.read();
// modRM.setRm16(alu.xor16(modRM.getRm16(), modRM.getR16())); } case 0x32-> {
// setCurrentInstructionName(()-> "XOR rb rmb"); modRM.read(); modRM.setR8(alu.xor8(modRM.getR8(),
// modRM.getRm8())); } case 0x33-> { setCurrentInstructionName(()-> "XOR rw rmw"); modRM.read();
// modRM.setR16(alu.xor16(modRM.getR16(), modRM.getRm16())); } case 0x34-> {
// setCurrentInstructionName(()-> "XOR AL mb"); state.setAL(alu.xor8(state.getAL(), nextUint8())); }
// case 0x35-> { setCurrentInstructionName(()-> "XOR AX mw"); state.setAX(alu.xor16(state.getAX(),
// nextUint16())); } case 0x36->handleInvalidOpcodeBecausePrefix(opcode); case 0x37-> {
// setCurrentInstructionName(()-> "AAA"); bool finalAuxillaryFlag = false; bool finalCarryFlag =
// false; if ((state.getAL() & 0x0F) > 9 || state.getAuxiliaryFlag()) { state.setAX(state.getAX() +
// 0x106); finalAuxillaryFlag = true; finalCarryFlag = true; } state.setAL(state.getAL() & 0x0F);

// // Undocumented behaviour alu.updateFlags8(state.getAL());
// state.setAuxiliaryFlag(finalAuxillaryFlag); state.setCarryFlag(finalCarryFlag); } case 0x38-> {
// setCurrentInstructionName(()-> "CMP rmb rb"); modRM.read(); alu.sub8(modRM.getRm8(),
// modRM.getR8()); } case 0x39-> { setCurrentInstructionName(()-> "CMP rmw rw"); modRM.read();
// alu.sub16(modRM.getRm16(), modRM.getR16()); } case 0x3A-> { setCurrentInstructionName(()-> "CMP
// rb rmb"); modRM.read(); alu.sub8(modRM.getR8(), modRM.getRm8()); } case 0x3B-> {
// setCurrentInstructionName(()-> "CMP rw rmw"); modRM.read(); alu.sub16(modRM.getR16(),
// modRM.getRm16()); } case 0x3C-> { setCurrentInstructionName(()-> "CMP AL ib");
// alu.sub8(state.getAL(), nextUint8()); } case 0x3D-> { setCurrentInstructionName(()-> "CMP AX
// iw"); alu.sub16(state.getAX(), nextUint16()); } case
// 0x3E->handleInvalidOpcodeBecausePrefix(opcode); case 0x3F-> { setCurrentInstructionName(()->
// "AAS"); bool finalAuxillaryFlag = false; bool finalCarryFlag = false; if ((state.getAL() & 0x0F)
// > 9 || state.getAuxiliaryFlag()) { state.setAX(state.getAX() - 6); state.setAH(state.getAH() -
// 1); finalAuxillaryFlag = true; finalCarryFlag = true; } state.setAL(state.getAL() & 0x0F);

// // Undocumented behaviour alu.updateFlags8(state.getAL());
// state.setAuxiliaryFlag(finalAuxillaryFlag); state.setCarryFlag(finalCarryFlag); } case 0x40,
// 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47-> { int regIndex = opcode & REG_INDEX_MASK;
// setCurrentInstructionName(()-> "INC " + state.getRegisters().getRegName(regIndex));
// state.getRegisters().setRegister(regIndex,
// alu.inc16(state.getRegisters().getRegister(regIndex))); } case 0x48, 0x49, 0x4A, 0x4B, 0x4C,
// 0x4D, 0x4E, 0x4F-> { int regIndex = opcode & REG_INDEX_MASK; setCurrentInstructionName(()-> "DEC
// " + state.getRegisters().getRegName(regIndex)); state.getRegisters().setRegister(regIndex,
// alu.dec16(state.getRegisters().getRegister(regIndex))); } case 0x50, 0x51, 0x52, 0x53, 0x54,
// 0x55, 0x56, 0x57-> { int regIndex = opcode & REG_INDEX_MASK; setCurrentInstructionName(()-> "PUSH
// " + state.getRegisters().getRegName(regIndex));
// stack.push(state.getRegisters().getRegister(regIndex)); } case 0x58, 0x59, 0x5A, 0x5B, 0x5C,
// 0x5D, 0x5E, 0x5F-> { int regIndex = opcode & REG_INDEX_MASK; setCurrentInstructionName(()-> "POP
// " + state.getRegisters().getRegName(regIndex)); state.getRegisters().setRegister(regIndex,
// stack.pop()); } case 0x60-> { // 80186 setCurrentInstructionName(()-> "PUSHA"); int sp =
// state.getSP(); stack.push(state.getAX()); stack.push(state.getCX()); stack.push(state.getDX());
// stack.push(state.getBX()); stack.push(sp); stack.push(state.getBP()); stack.push(state.getSI());
// stack.push(state.getDI()); } case 0x61-> { // 80186 setCurrentInstructionName(()-> "POPA");
// state.setDI(stack.pop()); state.setSI(stack.pop()); state.setBP(stack.pop());

// // not restoring SP stack.pop(); state.setBX(stack.pop()); state.setDX(stack.pop());
// state.setCX(stack.pop()); state.setAX(stack.pop()); } case 0x62, 0x63, 0x64, 0x65, 0x66,
// 0x67->handleInvalidOpcode(opcode); case 0x68-> { // 80186 int value = this.nextUint16();
// setCurrentInstructionName(()-> "PUSH " + ConvertUtils.toHex16(value)); stack.push(value); } case
// 0x69-> { modRM.read(); int value = this.nextUint16(); setCurrentInstructionName(()-> "IMUL16 rm16
// " + ConvertUtils.toHex16(value)); int result = alu.imul16(value, modRM.getRm16());
// modRM.setR16(result); } case 0x6A-> { // 80186 sign extend it to 16 bits int value =
// uint16(int8(this.nextUint8())); setCurrentInstructionName(()-> "PUSH " +
// ConvertUtils.toHex16(value)); stack.push(value); } case 0x6B-> { modRM.read(); int value =
// int8(this.nextUint8()); setCurrentInstructionName(()-> "IMUL8 rm16 " +
// ConvertUtils.toHex16(value)); int result = alu.imul16(value, modRM.getRm16());
// modRM.setR16(result); } case 0x6C, 0x6D, 0x6E, 0x6F-> // INSB, INSW, OUTSB, OUTSW
// processString(opcode); case 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7A,
// 0x7B, 0x7C, 0x7D, 0x7E, 0x7F->jcc( opcode); case 0x80, // GRP1 rmb ib, GRP1 rmw iw, GRP1 rmb ib,
// GRP1 rmw ib 0x81, 0x82, 0x83->grp1(opcode); case 0x84-> { setCurrentInstructionName(()-> "TEST
// rmb rb"); modRM.read(); alu.and8(modRM.getRm8(), modRM.getR8()); } case 0x85-> {
// setCurrentInstructionName(()-> "TEST rmw rw"); modRM.read(); alu.and16(modRM.getRm16(),
// modRM.getR16()); } case 0x86-> { setCurrentInstructionName(()-> "XCHG8"); modRM.read(); int
// value1 = modRM.getRm8(); int value2 = modRM.getR8(); modRM.setR8(value1); modRM.setRm8(value2); }
// case 0x87-> { setCurrentInstructionName(()-> "XCHG16"); modRM.read(); int value1 =
// modRM.getRm16(); int value2 = modRM.getR16(); modRM.setR16(value1); modRM.setRm16(value2); } case
// 0x88-> { setCurrentInstructionName(()-> "MOV rmb rb"); modRM.read(); modRM.setRm8(modRM.getR8());
// } case 0x89-> { setCurrentInstructionName(()-> "MOV rmw rw"); modRM.read();
// modRM.setRm16(uint16(modRM.getR16())); } case 0x8A-> { setCurrentInstructionName(()-> "MOV rb,
// rmb"); modRM.read(); modRM.setR8(modRM.getRm8()); } case 0x8B-> { setCurrentInstructionName(()->
// "MOV rw rmw"); modRM.read(); modRM.setR16(modRM.getRm16()); } case 0x8C-> {
// setCurrentInstructionName(()-> "MOV rmw sreg"); modRM.read();
// modRM.setRm16(uint16(modRM.getSegmentRegister())); } case 0x8D-> { setCurrentInstructionName(()->
// "LEA"); modRM.read(); modRM.setR16(uint16(modRM.getMemoryOffset())); } case 0x8E-> {
// setCurrentInstructionName(()-> "MOV sreg rmw"); modRM.read();
// modRM.setSegmentRegister(modRM.getRm16()); } case 0x8F-> { setCurrentInstructionName(()-> "POP
// rmw"); modRM.read(); modRM.setRm16(stack.pop()); } case 0x90->setCurrentInstructionName(()->
// "NOP"); case 0x91, 0x92, 0x93, 0x94, 0x95, 0x96, 0x97-> { int regIndex = opcode & REG_INDEX_MASK;
// setCurrentInstructionName(()-> "XCHG AX," + state.getRegisters().getRegName(regIndex)); int
// value1 = state.getAX(); int value2 = state.getRegisters().getRegister(regIndex);
// state.setAX(value2); state.getRegisters().setRegister(regIndex, value1); } case 0x98-> { //
// Convert byte to word setCurrentInstructionName(()-> "CBW");
// state.setAX(uint16(int8(state.getAL()))); } case 0x99-> { // Sign extend AX into DX (word to
// dword) setCurrentInstructionName(()-> "CWD"); if (state.getAX() >= 0x8000) { state.setDX(0xFFFF);
// } else { state.setDX(0); } } case 0x9A-> { // FAR CALL int ip = nextUint16(); int cs =
// nextUint16(); setCurrentInstructionName(()-> "FAR CALL"); farCall(state.getCS(), internalIp, cs,
// ip); }

// // Do nothing, this is to wait for the FPU which is not implemented case
// 0x9B->setCurrentInstructionName(()-> "WAIT"); case 0x9C-> { setCurrentInstructionName(()->
// "PUSHF"); stack.push(state.getFlags().getFlagRegister()); } case 0x9D-> {
// setCurrentInstructionName(()-> "POPF"); state.getFlags().setFlagRegister(stack.pop()); } case
// 0x9E-> { setCurrentInstructionName(()-> "SAHF"); state.getFlags().setFlagRegister(state.getAH());
// } case 0x9F-> { setCurrentInstructionName(()-> "LAHF");
// state.setAH(state.getFlags().getFlagRegister()); } case 0xA0-> { setCurrentInstructionName(()->
// "MOV AL moffs8"); state.setAL(memory.getUint8(getDsNextUint16Address()));
// staticAddressesRecorder.setCurrentAddressOperation(ValueOperation.READ, OperandSize.BYTE8); }
// case 0xA1-> { setCurrentInstructionName(()-> "MOV AX moffs16");
...
// case 0xA2-> { setCurrentInstructionName(()-> "MOV moffs8 AL");
...
// case 0xA3-> { setCurrentInstructionName(()-> "MOV moffs16 AX");
...
// case 0xA4, 0xA5, 0xA6, 0xA7->processString(opcode); case 0xA8-> { setCurrentInstructionName(()->
// "TEST AL ib"); alu.and8(state.getAL(), nextUint8()); } case 0xA9-> {
// setCurrentInstructionName(()-> "TEST AX iw"); alu.and16(state.getAX(), nextUint16()); } case
// 0xAA, 0xAB, 0xAC, 0xAD, 0xAE, 0xAF->processString(opcode); case 0xB0, 0xB1, 0xB2, 0xB3, 0xB4,
// 0xB5, 0xB6, 0xB7-> { int regIndex = opcode & REG_INDEX_MASK; setCurrentInstructionName(()-> "MOV
// " + state.getRegisters().getReg8Name(regIndex) + " ib");
// state.getRegisters().setRegisterFromHighLowIndex8(regIndex, nextUint8()); } case 0xB8, 0xB9,
// 0xBA, 0xBB, 0xBC, 0xBD, 0xBE, 0xBF-> { int regIndex = opcode & REG_INDEX_MASK;
// setCurrentInstructionName(()-> "MOV " + state.getRegisters().getRegName(regIndex) + " iw");
// state.getRegisters().setRegister(regIndex, nextUint16()); } case 0xC0-> { modRM.read(); int count
// = this.nextUint8(); int value = modRM.getRm8(); setCurrentInstructionName(()-> "SHL rmb " +
// count); modRM.setRm8(alu.shl8(value, count)); } case 0xC1-> { modRM.read(); int count =
// this.nextUint8(); int value = modRM.getRm16(); setCurrentInstructionName(()-> "SHL rmw " +
// count); modRM.setRm16(alu.shl16(value, count)); } case 0xC2-> { int numberOfBytesToPop =
// nextUint8(); setCurrentInstructionName(()-> "RET and pop " + numberOfBytesToPop + " bytes");
// nearRet(numberOfBytesToPop); } case 0xC3-> { setCurrentInstructionName(()-> "RET"); nearRet(0); }
// case 0xC4, 0xC5-> { // Copy segmented address that is in memory (32bits) into DS/ES and the
// specified register modRM.read(); modRM.setR16(memory.getUint16(modRM.getMemoryAddress())); int
// value = memory.getUint16(modRM.getMemoryAddress() + 2);
// this.getStaticAddressesRecorder().setCurrentAddressOperation(ValueOperation.READ,
// OperandSize.DWORD32); if (opcode == 0xC4) { // LES setCurrentInstructionName(()-> "LES rw md");
// state.setES(value); } else { // LDS setCurrentInstructionName(()-> "LDS rw md");
// state.setDS(value); } } case 0xC6-> { setCurrentInstructionName(()-> "MOV rmb ib"); modRM.read();
// modRM.setRm8(nextUint8()); } case 0xC7-> { setCurrentInstructionName(()-> "MOV rmw iw");
// modRM.read(); modRM.setRm16(nextUint16()); } case 0xC8, 0xC9->handleInvalidOpcode(opcode); case
// 0xCA-> { int numberOfBytesToPop = nextUint8(); setCurrentInstructionName(()-> "RETF and pop " +
// numberOfBytesToPop + " bytes"); farRet(numberOfBytesToPop); } case 0xCB-> {
// setCurrentInstructionName(()-> "RETF"); farRet(0); } case 0xCC-> { setCurrentInstructionName(()->
// "INT 3"); interrupt(3, false); } case 0xCD-> { setCurrentInstructionName(()-> "INT ib");
// interrupt(nextUint8(), false); } case 0xCE-> { setCurrentInstructionName(()-> "INTO"); if
// (state.getOverflowFlag()) { interrupt(4, false); } } case 0xCF-> { setCurrentInstructionName(()->
// "IRET"); interruptRet(); } case 0xD0, 0xD1, 0xD2, 0xD3-> // GRP2 rmb 1, GRP2 rmw 1, GRP2 rmb CL,
// GRP2 rmw CL grp2(opcode); case 0xD4-> { setCurrentInstructionName(()-> "AAM ib"); int v1 =
// state.getAL(); int v2 = nextUint8(); if (v2 == 0) { handleDivisionError(); break; } int result =
// v1 % v2; state.setAH(v1 / v2); state.setAL(result); alu.updateFlags8(result); } case 0xD5-> {
// setCurrentInstructionName(()-> "AAD ib"); int result = uint8(state.getAL() + state.getAH() *
// nextUint8()); state.setAL(result); state.setAH(0); state.getFlags().setFlagRegister(0);
// alu.updateFlags8(result); } case 0xD6->handleInvalidOpcode(opcode); case 0xD7-> {
// setCurrentInstructionName(()-> "XLAT"); int address = modRM.getAddress(SegmentRegisters.DS_INDEX,
// state.getBX()) + state.getAL(); state.setAL(memory.getUint8(address)); } case
// 0xD8->handleInvalidOpcode(opcode); case 0xD9-> { modRM.read(); int groupIndex =
// modRM.getRegisterIndex(); switch (groupIndex) { case 0x7-> { // Set the control word to the value
// expected after init since FPU is not supported. modRM.setRm16(0x37F);
// setCurrentInstructionName(()-> "FNSTCW"); } default -> throw new
// InvalidGroupIndexException(machine, groupIndex); } } case 0xDA->handleInvalidOpcode(opcode); case
// 0xDB-> { int opCodeNextByte = nextUint8(); if (opCodeNextByte != 0xE3) { int fullOpCode = (opcode
// << 8) | opCodeNextByte; handleInvalidOpcode(fullOpCode); } setCurrentInstructionName(()-> "FNINIT");
...
// // Upper part of the result goes in AH state.setAH(uint8(result >> 8));
// state.setAL(uint8(result)); } case 5-> { setCurrentInstructionName(()-> "IMUL8"); int result =
// alu.imul8(state.getAL(), modRM.getRm8());
...
// // Upper part of the result goes in DX state.setDX(result >>> 16); state.setAX(result); } case
// 5-> { setCurrentInstructionName(()-> "IMUL16"); int result = alu.imul16(state.getAX(), modRM.getRm16());

// // Upper part of the result goes in DX state.setDX(result >>> 16); state.setAX(result); } case
// 6-> { setCurrentInstructionName(()-> "DIV16"); int v1 = (state.getDX() << 16) | state.getAX();
// int v2 = modRM.getRm16(); int result = alu.div16(v1, v2); if (result == null) {
// handleDivisionError(); break; } state.setAX(result); state.setDX((int)(uint32(v1) % uint32(v2)));
// } case 7-> { setCurrentInstructionName(()-> "IDIV16");
