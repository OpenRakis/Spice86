...
internal partial class MainWindow : Window {
    ...
    private void MainWindow_Loaded(object? sender, Avalonia.Interactivity.RoutedEventArgs e) {
        Dispatcher.UIThread.Post(() => {
            Console.WriteLine("[WARN] MainWindow: MainWindow_Loaded called");
            if (DataContext is MainWindowViewModel viewModel) {
                // Wire up OpenGL frame updates
                viewModel.UpdateOpenGlFrame += OnUpdateOpenGlFrame;
                Console.WriteLine("[WARN] MainWindow: UpdateOpenGlFrame event wired");
                
                // Track window size changes for shader selection
                this.PropertyChanged += OnWindowPropertyChanged;
                Console.WriteLine("[WARN] MainWindow: PropertyChanged event wired");
                
                // Set initial host output resolution
                UpdateHostOutputResolution();
                
                Console.WriteLine($"[WARN] MainWindow: OpenGlVideo is {(OpenGlVideo != null ? "NOT NULL" : "NULL")}");
    ...
    private void OnWindowPropertyChanged(object? sender, AvaloniaPropertyChangedEventArgs e) {
        if (e.Property == ClientSizeProperty || e.Property == WindowStateProperty) {
            Console.WriteLine($"[WARN] MainWindow: Window property changed - {e.Property.Name}");
            UpdateHostOutputResolution();
        }
    }
    ...
    private void UpdateHostOutputResolution() {
        if (DataContext is MainWindowViewModel viewModel) {
            ...
            Console.WriteLine($"[WARN] MainWindow: UpdateHostOutputResolution - {outputWidth}x{outputHeight}");
    ...
    private void OnUpdateOpenGlFrame(uint[] frameBuffer, int width, int height) {
        Console.WriteLine($"[WARN] MainWindow: OnUpdateOpenGlFrame called - OpenGlVideo={(OpenGlVideo != null ? "exists" : "NULL")}, width={width}, height={height}, buffer.Length={frameBuffer.Length}");
