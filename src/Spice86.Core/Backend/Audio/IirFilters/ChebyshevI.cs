 ...
 */
internal class ChebyshevI : Cascade {
    class AnalogLowPass : LayoutBase {
        ...
        public void Design(double rippleDb) {
            ...
            if ((_nPoles & 1) == 1) {
                Add(new Complex(sinhV0, 0), new Complex(
                        double.PositiveInfinity, 0));
                SetNormal(0, 1);
            } else {
    ...
    private void SetupLowPass(int order, double sampleRate,
        ...
        var mAnalogProto = new AnalogLowPass(order);
        mAnalogProto.Design(rippleDb);

        var mDigitalProto = new LayoutBase(order);

        new LowPassTransform(cutoffFrequency / sampleRate, mDigitalProto,
                mAnalogProto);

        SetLayout(mDigitalProto, directFormType);
    }
