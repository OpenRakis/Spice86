...
public static class AspectRatioHelper {
    ...
    /// When aspect correction is enabled, duplicates every 5th line to achieve 1.2x vertical stretch (200 → 240 lines).
    /// </summary>
    /// <param name="needsAspectCorrection">Whether the current video mode requires aspect ratio correction (determined by caller based on video mode).</param>
    ...
    public static bool ShouldDuplicateLine(bool needsAspectCorrection, int nativeHeight, int currentLine) {
        if (needsAspectCorrection) {
            // Duplicate every 5th line (lines 4, 9, 14, ...) for 1.2x vertical stretch
    ...
    /// </summary>
    /// <param name="needsAspectCorrection">Whether the current video mode requires aspect ratio correction.</param>
    ...
    public static int CalculateLinesToDraw(bool needsAspectCorrection, int nativeHeight, int currentLine, int baseLinesPerScanline) {
        return ShouldDuplicateLine(needsAspectCorrection, nativeHeight, currentLine) 
