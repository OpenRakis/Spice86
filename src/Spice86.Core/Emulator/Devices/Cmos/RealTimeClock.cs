...
/// <item>
/// <description>
/// <b>CMOS configuration registers:</b> Only a subset of configuration registers are implemented; many are stubbed because they are not documented. Unimplemented registers may return 0 or fixed values.
/// </description>
/// </item>
/// </list>
/// </para>
/// </summary>
public sealed class RealTimeClock : DefaultIOPortHandler {
    ...
    public RealTimeClock(State state, IOPortDispatcher ioPortDispatcher, DualPic dualPic,
        ...
        // Initialize RTC control registers with defaults matching real hardware
    ...
    /// </para>
    /// <para>
    /// Note: Contraption Zack relies on the 0xC0 flag pattern for its music.
    /// </para>
    /// </summary>
    private void ProcessPendingPeriodicEvents() {
        ...
        if (nowMs >= _nextPeriodicTriggerMs) {
            ...
            // Contraption Zack relies on both IRQF and PF being set in Status Register C to process timer interrupts.
