...
public sealed class PcSpeaker : DefaultIOPortHandler, IDisposable {
    ...
    public PcSpeaker(SoftwareMixer softwareMixer,
        ...
        pit8254Counter.FrequencyChangedEvent += OnFrequencyChange;
    ...
    private void OnFrequencyChange(object? source, EventArgs e) {
        if (_loggerService.IsEnabled(Serilog.Events.LogEventLevel.Verbose)) {
    ...
    private void Dispose(bool disposing) {
        if (!_disposed) {
            if (disposing) {
                _pit8254Counter.FrequencyChangedEvent -= OnFrequencyChange;
                _endPlayback = true;
