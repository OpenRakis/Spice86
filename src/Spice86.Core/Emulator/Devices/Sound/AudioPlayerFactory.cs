...
public class AudioPlayerFactory {
    ...
    /// Uses WASAPI on Windows and SDL on other platforms (Linux, macOS).
    ...
    /// <param name="suggestedLatency">Unused parameter kept for API compatibility.</param>
    ...
    public AudioPlayer CreatePlayer(int sampleRate = 48000, int framesPerBuffer = 0,
        ...
        if (_audioEngine == AudioEngine.CrossPlatform) {
            AudioPlayer? player = TryCreateCrossPlatformPlayer(sampleRate, framesPerBuffer);
            if (player != null) {
                return player;
            }
    ...
    private AudioPlayer? TryCreateCrossPlatformPlayer(int sampleRate, int framesPerBuffer) {
        try {
            ...
            return new CrossPlatformAudioPlayer(format, backend, bufferFrames);
