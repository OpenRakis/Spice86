...
/// </summary>
public sealed class OPL2 : DefaultIOPortHandler, IDisposable {
    ...
    private double GetCpuBasedTime() {
        ...
        long wholeMs = _state.Cycles / (actualCyclesPerMs is 0 ? 1 : actualCyclesPerMs);

        // Calculate fractional progress within current millisecond
        // This is equivalent to DOSBox's PIC_TickIndex()
        double fractionalMs = (_state.Cycles % (actualCyclesPerMs is 0 ? 1 : actualCyclesPerMs)) /
            (actualCyclesPerMs is 0 ? 1 : (double)actualCyclesPerMs);
    ...
    private void FillBuffer(Span<float> playBuffer) {
        lock (_lock) {
            ...
            while (framesRemaining > 0 && _fifo.Count > 0) {
                (float Left, float Right) = _fifo.Dequeue();
                playBuffer[bufferIndex++] = Left;
                playBuffer[bufferIndex++] = Right;
                framesRemaining--;
            }
            ...
            _lastRenderedMs = GetCpuBasedTime();
        }
    }
