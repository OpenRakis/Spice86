...
public class Opl : DefaultIOPortHandler, IDisposable {
    ...
    // Exposes real-time counters, histograms, and gauges on the "Spice86.Opl"
    // meter. Attach a MeterListener in tests or use dotnet-counters for live
    ...
    internal static readonly Counter<long> AudioCallbackCount =
        OplMeter.CreateCounter<long>("opl.audio_callback.count",
            description: "Number of AudioCallback invocations (mixer thread)");
    internal static readonly Counter<long> AudioCallbackFramesRequested =
        OplMeter.CreateCounter<long>("opl.audio_callback.frames_requested",
            description: "Total frames requested by the mixer thread");
    internal static readonly Counter<long> AudioCallbackFifoFramesServed =
        OplMeter.CreateCounter<long>("opl.audio_callback.fifo_frames_served",
            description: "Frames served from the FIFO queue in AudioCallback");
    internal static readonly Counter<long> AudioCallbackDirectRenderFrames =
        OplMeter.CreateCounter<long>("opl.audio_callback.direct_render_frames",
            description: "Frames rendered directly (FIFO empty) in AudioCallback");
    internal static readonly Histogram<double> AudioCallbackDurationMs =
        OplMeter.CreateHistogram<double>("opl.audio_callback.duration_ms",
            "ms", "Wall-clock duration of each AudioCallback invocation");
    internal static readonly Counter<long> AudioCallbackFifoStarvations =
        OplMeter.CreateCounter<long>("opl.audio_callback.fifo_starvations",
            description: "Times AudioCallback had to direct-render (FIFO empty)");

    // ── RenderUpToNow metrics ─────────────────────────────────────────
    internal static readonly Counter<long> RenderUpToNowCount =
        OplMeter.CreateCounter<long>("opl.render_up_to_now.count",
            description: "Number of RenderUpToNow invocations (CPU thread)");
    internal static readonly Counter<long> RenderUpToNowFramesGenerated =
        OplMeter.CreateCounter<long>("opl.render_up_to_now.frames_generated",
            description: "Total FIFO frames generated by RenderUpToNow");
    internal static readonly Histogram<double> RenderUpToNowGapMs =
        OplMeter.CreateHistogram<double>("opl.render_up_to_now.gap_ms",
            "ms", "Time gap (now - _lastRenderedMs) seen by RenderUpToNow");
    internal static readonly Counter<long> RenderUpToNowWakeUps =
        OplMeter.CreateCounter<long>("opl.render_up_to_now.wake_ups",
            description: "Times RenderUpToNow triggered a channel WakeUp");

    // ── FIFO metrics ──────────────────────────────────────────────────
    internal static readonly Histogram<int> FifoDepthAtCallback =
        OplMeter.CreateHistogram<int>("opl.fifo.depth_at_callback",
            description: "FIFO queue depth when AudioCallback starts");

    // ── Timing metrics ────────────────────────────────────────────────
    internal static readonly Histogram<double> LastRenderedMsAtCallback =
        OplMeter.CreateHistogram<double>("opl.timing.last_rendered_ms_at_callback",
            "ms", "_lastRenderedMs value when AudioCallback starts");
    internal static readonly Histogram<double> ClockElapsedMsAtCallback =
        OplMeter.CreateHistogram<double>("opl.timing.clock_elapsed_ms_at_callback",
            "ms", "_clock.ElapsedTimeMs value when AudioCallback starts");
    internal static readonly Histogram<double> ClockElapsedMsAtRenderUpToNow =
    ...
    private void RenderUpToNow() {
        ...
        double gap = now - _lastRenderedMs;
        ...
        RenderUpToNowFramesGenerated.Add(framesGenerated);
    }
    ...
    private void AudioCallback(int framesRequested) {
        ...
        try {
            ...
            LastRenderedMsAtCallback.Record(_lastRenderedMs);
            ClockElapsedMsAtCallback.Record(_clock.ElapsedTimeMs);
            ...
            AudioCallbackFifoFramesServed.Add(fifoServed);

            // If the queue's run dry, render the remainder and sync-up our time datum
            if (framesRemaining > 0) {
                AudioCallbackFifoStarvations.Add(1);
            }
            long directRendered = 0;
            ...
            AudioCallbackDirectRenderFrames.Add(directRendered);
        ...
        AudioCallbackDurationMs.Record(elapsedMs);
    }
