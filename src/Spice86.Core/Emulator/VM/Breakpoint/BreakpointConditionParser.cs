...
/// </summary>
public class BreakpointConditionParser {
    ...
    private ValueNode ParseLogicalOr() {
        ...
        while (Match("||")) {
            left = new BinaryOperationNode(DataType.UINT32, left, BinaryOperation.LOGICAL_OR, ParseLogicalAnd());
        }
        return left;
    }
    ...
    private ValueNode ParseLogicalAnd() {
        ...
        while (Match("&&")) {
            left = new BinaryOperationNode(DataType.UINT32, left, BinaryOperation.LOGICAL_AND, ParseBitwiseOr());
        }
        return left;
    }
    ...
    private ValueNode ParseEquality() {
        ...
        while (true) {
            if (Match("==")) {
                left = new BinaryOperationNode(DataType.UINT32, left, BinaryOperation.EQUAL, ParseRelational());
            } else if (Match("!=")) {
                left = new BinaryOperationNode(DataType.UINT32, left, BinaryOperation.NOT_EQUAL, ParseRelational());
            } else {
                break;
            }
        }
    ...
    private ValueNode ParseRelational() {
        ...
        while (true) {
            if (Match("<=")) {
                left = new BinaryOperationNode(DataType.UINT32, left, BinaryOperation.LESS_THAN_OR_EQUAL, ParseShift());
            } else if (Match(">=")) {
                left = new BinaryOperationNode(DataType.UINT32, left, BinaryOperation.GREATER_THAN_OR_EQUAL, ParseShift());
            } else if (CurrentChar() == '<') {
                Advance();
                left = new BinaryOperationNode(DataType.UINT32, left, BinaryOperation.LESS_THAN, ParseShift());
            } else if (CurrentChar() == '>') {
                Advance();
                left = new BinaryOperationNode(DataType.UINT32, left, BinaryOperation.GREATER_THAN, ParseShift());
            } else {
                break;
            }
        }
    ...
    private ValueNode ParseUnary() {
        if (CurrentChar() == '!') {
            Advance();
            return new UnaryOperationNode(DataType.UINT32, UnaryOperation.NOT, ParseUnary());
        }
        if (CurrentChar() == '-') {
            Advance();
