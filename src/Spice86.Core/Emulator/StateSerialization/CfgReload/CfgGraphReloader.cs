...
internal sealed class CfgGraphReloader {
    ...
    public CfgGraphReloader(CfgCpu cfgCpu, State state, CfgNodeExecutionCompiler executionCompiler) {
        _state = state;
        ...
        _idAllocator = cfgCpu.CfgNodeFeeder.IdAllocator;
    }
    ...
    private void RestoreEdges(CfgReloadDump dump, Dictionary<int, ICfgNode> nodesById) {
        // Group edges by source so each node's successor set + cache can be finalized once.
        IEnumerable<IGrouping<int, CfgReloadEdgeInfo>> bySource = dump.Edges.GroupBy(edge => edge.From);

        foreach (IGrouping<int, CfgReloadEdgeInfo> group in bySource) {
            ICfgNode from = ResolveNode(nodesById, group.Key);
            foreach (CfgReloadEdgeInfo edge in group) {
