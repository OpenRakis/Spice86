...
public class DosFileControlBlockManager {
    ...
    public bool RenameFile(uint fcbAddress) {
        ...
        string? oldHostPath = _dosDriveManager.GetCurrentDosPathResolver().GetFullHostPathFromDosOrDefault(oldName);
        string? newHostPath = _dosDriveManager.GetCurrentDosPathResolver().PrefixWithHostDirectory(newName);

        if (string.IsNullOrEmpty(oldHostPath) || string.IsNullOrEmpty(newHostPath)) {
            return false;
        }
    ...
    private void UpdateFcbFromFileInfo(DosFileControlBlock fcb, string dosFileName) {
        string? hostPath = _dosDriveManager.GetCurrentDosPathResolver().GetFullHostPathFromDosOrDefault(dosFileName);
