...
internal class DosExecParameterBlock : DosMemoryControlBlock {
    ...
    /// <summary>
    /// Gets or the sets the segment of environement for the child process (0x0: duplicate the current PSP's environment)
    /// </summary>
    ...
    /// </summary>
    public uint CommandTail { get => UInt32[0x2]; set { UInt32[0x2] = value; } }

    /// <summary>
    /// Gets or sets the address of the first file control block. Unopened FCB to be copied to the child PSP.
    /// </summary>
    public uint FirstFileControlBlock { get => UInt32[0x6]; set { UInt32[0x6] = value; } }

    /// <summary>
    /// Gets or sets the address of the second file control block. Unopened FCB to be copied to the child PSP.
    /// </summary>
    public uint SecondFileControlBlock { get => UInt32[0x10]; set { UInt32[0x10] = value; } }
