...
internal sealed partial class DosBatchExecutionEngine {
    ...
    internal bool HandleMount(string arguments) {
        ...
        if (string.IsNullOrWhiteSpace(trimmed)) {
            WriteMountedDriveList();
            return false;
        }

        string[] parts = BatchArgumentParser.SplitWithQuotes(trimmed);
        if (TryHandleMountedDriveUnmount("MOUNT", parts)) {
            return false;
        }
        if (parts.Length < 2) {
    ...
    internal bool HandleImgMount(string arguments) {
        ...
        string[] parts = BatchArgumentParser.SplitWithQuotes(trimmed);
        if (TryHandleMountedDriveUnmount("IMGMOUNT", parts)) {
            return false;
        }
        if (parts.Length < 2) {
    ...
    private bool TryHandleMountedDriveUnmount(string commandName, string[] parts) {
        ...
        for (int i = 0; i < parts.Length; i++) {
            if (parts[i].Equals("-u", StringComparison.OrdinalIgnoreCase)) {
                hasUnmountFlag = true;
                break;
            }
        }
        ...
        if (parts.Length != 2) {
            WriteToStandardOutput($"{commandName}: usage {commandName} -u <drive>\r\n");
            return true;
        }
        ...
        if (parts[0].Equals("-u", StringComparison.OrdinalIgnoreCase)) {
            driveSpec = parts[1];
        } else if (parts[1].Equals("-u", StringComparison.OrdinalIgnoreCase)) {
            driveSpec = parts[0];
        } else {
            WriteToStandardOutput($"{commandName}: usage {commandName} -u <drive>\r\n");
            return true;
        ...
        bool removedCdDrive = _mscdex.RemoveDrive(driveLetter);
        try {
            _driveManager.Unmount(driveLetter);
        ...
        EnsureCurrentDriveAfterUnmount(driveLetter);
        WriteToStandardOutput($"Drive {driveLetter}: unmounted\r\n");
        return true;
    }

    private void EnsureCurrentDriveAfterUnmount(char removedDriveLetter) {
        if (_driveManager.CurrentDrive.DriveLetter != removedDriveLetter) {
            return;
        }
    ...
    private bool HasListedMountedDrive(char driveLetter) {
        if (TryGetCdRomDriveEntry(driveLetter, out MscdexDriveEntry? _)) {
            return true;
        }

        if (_driveManager.TryGetFloppyDrive(driveLetter, out FloppyDiskDrive? _)) {
            return true;
        }
    ...
    private bool TryGetCdRomDriveEntry(char driveLetter, out MscdexDriveEntry? driveEntry) {
        char upper = char.ToUpperInvariant(driveLetter);
        for (int i = 0; i < _mscdex.Drives.Count; i++) {
            MscdexDriveEntry current = _mscdex.Drives[i];
            if (char.ToUpperInvariant(current.DriveLetter) == upper) {
                driveEntry = current;
                return true;
            }
        }
