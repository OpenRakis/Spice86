...
public class AstBuilder {
    public BitWidth Size(int size) {
        return size switch {
            8 => Shared.Emulator.Memory.BitWidth.BYTE_8,
            16 => Shared.Emulator.Memory.BitWidth.WORD_16,
            32 => Shared.Emulator.Memory.BitWidth.DWORD_32,
    ...
    public ValueNode ToSegmentedPointer(DataType targetDataType, int segmentRegisterIndex, BitWidth registerSize, int registerIndex) {
        return ToSegmentedPointer(targetDataType, new SegmentRegisterNode(segmentRegisterIndex),
            new RegisterNode(new DataType(registerSize, false), registerIndex));
    }
    ...
    public ValueNode Data(DataType dataType) {
        return new RegisterNode(dataType, (int)RegisterIndex.DxIndex);
    }

    public ValueNode Base(DataType dataType) {
        return new RegisterNode(dataType, (int)RegisterIndex.BxIndex);
    }

    public ValueNode Counter(DataType dataType) {
        return new RegisterNode(dataType, (int)RegisterIndex.CxIndex);
    }
    ...
    private ValueNode PlusRegs16(RegisterIndex registerIndex1, RegisterIndex registerIndex2) {
        return new BiOperationWithResultNode(DataType.UINT16, Reg16(registerIndex1), Operation.PLUS,
    ...
    private ValueNode? BiOperationWithResultNode(DataType dataType,
        ...
        if (parameter1 != null && parameter2 != null) {
            return new BiOperationWithResultNode(dataType, parameter1, operation, parameter2);
        }
