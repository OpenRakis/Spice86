...
public class CallFlowHandler {
    ...
    public void Call(CallType callType, SegmentedAddress entryAddress, SegmentedAddress? expectedReturnAddress, CfgInstruction? call) {
        FunctionCall currentFunctionCall = new(callType, entryAddress, expectedReturnAddress, _state.StackSegmentedAddress, call);
    ...
    private void PopCallStackUntilMatchPhysical() {
        ...
        while (toPop-- > 0) {
            ...
            if (functionCall.Initiator != null) {
                functionCall.Initiator.NotARegularReturn = true;
            }
        }
    }
