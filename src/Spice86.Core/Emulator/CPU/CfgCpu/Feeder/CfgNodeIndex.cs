...
public class CfgNodeIndex : InstructionReplacer {
    ...
    public void Insert(CfgInstruction node) {
        if (!_index.TryGetValue(node.Address, out List<CfgInstruction>? atAddress)) {
            atAddress = new List<CfgInstruction>();
    ...
    /// <summary>
    /// Removes <paramref name="node"/> from the index. Used only by the discard sweep;
    /// does not touch graph edges.
    /// </summary>
    public void Remove(CfgInstruction node) {
        if (!_index.TryGetValue(node.Address, out List<CfgInstruction>? atAddress)) {
            return;
        }
        atAddress.Remove(node);
    ...
    public override void ReplaceInstruction(CfgInstruction oldInstruction, CfgInstruction newInstruction) {
        if (!_index.TryGetValue(oldInstruction.Address, out List<CfgInstruction>? atAddress)) {
            return;
        }
