...
public class LxsParser : BaseInstructionParser {
    ...
    public CfgInstruction Parse(ParsingContext context, InstructionOperation displayOp, SegmentRegisterIndex segmentRegisterIndex) {
        ...
        BinaryOperationNode assignR = _astBuilder.Assign(dataType, rNode, memValue);
        ...
        BinaryOperationNode assignSeg = _astBuilder.Assign(DataType.UINT16, segRegNode, segPointer);
        ...
        if (segmentRegisterIndex == SegmentRegisterIndex.SsIndex) {
            ...
            execAst = _astBuilder.WithIpAdvancement(instr, cachedOffset, assignR, assignSeg, setInterruptShadowingNode);
        } else {
            execAst = _astBuilder.WithIpAdvancement(instr, cachedOffset, assignR, assignSeg);
        }
