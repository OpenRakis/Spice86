...
public class MovSregRm16 : InstructionWithModRm {
    ...
    protected override IVisitableAstNode BuildExecutionAst(AstBuilder builder) {
        ...
        if (ModRmContext.RegisterIndex == (uint)SegmentRegisterIndex.SsIndex) {
            MethodCallNode setInterruptShadowing = new MethodCallNode(null, nameof(Spice86.Core.Emulator.CPU.CfgCpu.InstructionExecutor.InstructionExecutionHelper.SetInterruptShadowing));
