...
public class Stack {
    ...
    /// upper half; on a 32-bit stack ESP becomes the full new value.
    /// </summary>
    public void PopAll32() {
        ...
        if (StackAddressIs32Bit) {
            _state.ESP = offset;
        } else if (_state.CpuMode == CpuMode.Protected) {
            _state.ESP = (_state.ESP & 0xFFFF0000u) | offset;
        } else {
            // Real-mode/V86 80386 POPAD preserves the discarded slot's upper half in ESP.
    ...
    public void Enter(ushort storageSize, byte level, bool operandSize32) {
        ...
        if (!StackAddressIs32Bit && _state.CpuMode == CpuMode.Protected) {
        ...
        if (StackAddressIs32Bit) {
            ...
            if (operandSize32 && _state.CpuMode != CpuMode.Protected) {
