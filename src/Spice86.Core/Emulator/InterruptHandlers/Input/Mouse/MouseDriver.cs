...
public class MouseDriver : IMouseDriver {
    ...
    public MouseDriver(Cpu cpu, Memory memory, IMouseDevice mouseDevice, IMainWindowViewModel? gui, IVgaFunctionality vgaFunctions, ILoggerService loggerService) {
        _cpu = cpu;
        _memory = memory;
        _state = cpu.State;
