...
public sealed class ExpandedMemoryManager : InterruptHandler, IMemoryDevice {
    ...
    public Span<byte> GetSpan(int address, int length) {
        uint emmPageFrameAddress = MemoryUtils.ToPhysicalAddress(EmmPageFrameSegment, 0);
        int relativeAddress = (int)(address - emmPageFrameAddress);
    ...
    /// <param name="handle">The assigned handle id.</param>
    /// <param name="logicalPage">The logical page id.</param>
    private void RegisterEmmMapping(int physicalPage, ushort handle, ushort logicalPage) {
    ...
    /// </summary>
    /// <param name="mapping">The EMM Mapping to unmap from main memory</param>
    private void UnregisterEmmMapping(EmmMapping mapping) {
        if (_loggerService.IsEnabled(LogEventLevel.Debug)) {
    ...
    private byte EmmMapPage(ushort physicalPage, ushort handle, ushort logicalPage) {
        ...
        if (logicalPage < EmmHandles[handle].LogicalPage) {
            /* Mapping it is */
            RegisterEmmMapping(physicalPage, handle, logicalPage);
            return EmmStatus.EmmNoError;
        } else {
    ...
    public byte SavePageMap(ushort handle) {
        ...
        for (int i = 0; i < EmmMappings.Length; i++) {
            EmmHandles[handle].PageMap[i].LogicalPage = EmmMappings[i].LogicalPage;
    ...
    public byte RestorePageMap(ushort handle) {
        ...
        EmmHandles[handle].SavePageMap = false;
        for (int i = 0; i < EmmMappings.Length; i++) {
            EmmMappings[i].LogicalPage = EmmHandles[handle].PageMap[i].LogicalPage;
    ...
    private byte RestoreMappingTable() {
        ...
        for (int i = 0; i < EmmSegmentMappings.Length; i++) {
            ...
            EmmMapSegment(i << 10, EmmSegmentMappings[i].Handle, EmmSegmentMappings[i].LogicalPage);
        }
        for (ushort i = 0; i < EmmMappings.Length; i++) {
            ushort handle = EmmMappings[i].Handle;
            EmmMapPage(i, handle, EmmMappings[i].LogicalPage);
        }
        return EmmStatus.EmmNoError;
    }
    ...
    private byte EmmMapSegment(int segment, ushort handle, ushort logicalPage) {
        ...
        if (logicalPage == EmmNullPage) {
            /* Unmapping */
            UnregisterEmmMapping(physicalPage is >= 0 and < EmmMaxPhysicalPages ? mapping : emmSegmentMapping);
            return EmmStatus.EmmNoError;
        }
        if (!IsValidHandle(handle)) {
            return EmmStatus.EmmInvalidHandle;
        ...
        // Mapping
        RegisterEmmMapping(physicalPage is >= 0 and < EmmMaxPhysicalPages ? physicalPage : segmentMapping, handle, logicalPage);
        return EmmStatus.EmmNoError;
    }
        ...
        switch (operation) {
            ...
            case 0x02:	/* Save and Restore Page Map */
                ...
                for (int i = 0; i < EmmMappings.Length; i++) {
                    ...
                    EmmMappings[i].LogicalPage = _memory.GetUint16((uint) (offset + i + sizeof(ushort)));
    ...
    public byte HandleFunctions(byte operation) {
        ...
        switch (operation) {
            ...
            case 0x01: /* Search for a handle name */
                string name = _memory.GetZeroTerminatedString(MemoryUtils.ToPhysicalAddress(_state.DS, _state.SI), 8);
                for (handle = 0; handle < EmmMaxHandles; handle++) {
                    if (EmmHandles[handle].LogicalPage == EmmNullHandle ||
