...
using Spice86.Aeon.Emulator.Video.Registers.AttributeController;
...
public sealed class AttributeControllerRegisters {
    ...
    public byte ReadRegister(AttributeControllerRegister address) {
        if (address is >= AttributeControllerRegister.FirstPaletteEntry and <= AttributeControllerRegister.LastPaletteEntry)
    ...
    public void WriteRegister(AttributeControllerRegister address, byte value) {
        if (address is >= AttributeControllerRegister.FirstPaletteEntry and <= AttributeControllerRegister.LastPaletteEntry) {
            InternalPalette[(byte)address] = (byte)(value & 0x3F);
        } else {
            switch (address) {
