...
- Phase 4 (CUE/BIN Audio & Codec Dispatch): In Progress (50%) - Atom 1 landed: substantial vertical slice covering 4a + 4b. New `CueFileType` enum (BINARY/MOTOROLA/WAVE/AIFF/MP3/FLAC/OGG/OPUS) and `CueEntry.FileType` property (defaults to `Binary` for legacy CUE sheets that omit the type token). `CueSheetParser` now captures the trailing FILE-directive type token via a dedicated `ParseFileType` regex and threads it onto every emitted entry. New sealed `WavAudioFile` reads RIFF/WAVE PCM headers, walking chunks to locate `fmt ` + `data`, validating CDDA compliance (44100 Hz / 2 ch / 16-bit / PCM format tag 1) and rejecting non-conforming streams with `InvalidDataException`. New sealed `WindowedDataSource` (`IDataSource`) exposes a contiguous byte window into a backing source, used to project a WAV's PCM payload as raw 2352-byte CD audio sectors with zero copy. `CueBinImage` constructor refactored: `_sources` is now `Dictionary<string, IDataSource>` with a parallel `_ownedDisposables` list; `OpenSource(filePath, fileType)` dispatches BINARY → `FileBackedDataSource` and WAVE → `WavAudioFile`-driven `WindowedDataSource` over a `FileBackedDataSource`. End-to-end: a CUE referencing `FILE "track.wav" WAVE TRACK 01 AUDIO INDEX 01 00:02:00` now opens, decodes the WAV header, and serves PCM bytes back through `Read(lba, span, AudioRaw2352)` matching the WAV payload byte-for-byte. 6 RED-then-GREEN tests in `CueWavAudioTests` cover WAVE type extraction, BINARY default, CDDA WAV header parsing (offset/length/format), non-CDDA rejection, end-to-end audio sector readback (two sectors, distinctive payloads), and legacy BINARY regression. 101/101 storage + 2142/2142 main tests green. Atom 2 remaining: Phase 4c INDEX 00 pregap layout (`CueTrackLayout` + `CueFrameMapper`). Atom 3 remaining: Phase 4d Subchannel-Q synthesis (MSCDEX integration, BCD encoding). MP3/FLAC/OGG codec backends deferred until a transcoding/decoder dependency is selected.
...
- **Overall progress (effort-weighted, excluding skipped Phase 2): 90%**.
- **Per-phase progression:**
  ...
  - Phase 4: **50%** (4a + 4b landed: WAV codec + CUE FILE-type dispatch; 4c INDEX 00 pregap and 4d Subchannel-Q remaining)
  - Phase 6: **100%**
  - Phase 7: **100%**
