...
- Phase 2 (LFN/VFAT): Done (100%). Atom A landed the read path (codec + paired directory reader). Atom B landed the write path: `MutableFatDirectoryEntry.LongName` property (defaults to empty, falls back to legacy short-only write when blank), sealed `Dos83AliasGenerator.GenerateAlias(longName, existingShortNames)` producing canonical Windows-style `BASENA~1.EXT` aliases (strips spaces, dots, and disallowed characters; truncates the basename to 6 chars for `~1..~9`; shrinks the prefix to 5 chars and emits `~10..~99` on collision; uppercases via culture-invariant rules; ASCII-cap-3 on the extension), and `DirectoryWriter.WriteEntryWithLongName(directorySectors, entry, fatTable)` which packs the 8.3 short name into 11 bytes, computes the slot checksum via the codec, encodes the long name to N slots, locates the first contiguous run of N+1 free directory slots (treating both 0x00 and 0xE5 markers as free), writes the LFN slots in disc order via `LongFileNameCodec.WriteSlot`, then writes the short entry through the existing `WriteEntry` path so FAT-chain bookkeeping is preserved. 11 new tests in `tests/Spice86.Storage.Tests/FileSystem/Directory/LongFileName/` cover alias generation (simple `~1`, 6-char truncation, no-extension form, space/dot stripping, collision bumping, past-9 base shrink with two-digit counter, disallowed-character drop) and writer integration (two-slot round-trip through `VfatDirectoryReader`, single-slot LFN+short layout, contiguous-run skipping populated entries, empty-long-name fallback to short-only). New `Spice86.Shared.Emulator.Storage.FileSystem.Directory.LongFileName` namespace adds sealed immutable `VfatLfnEntry` (1-based ordinal, IsLast flag, 8.3-name checksum, 13-char UCS-2 fragment with defensive clone; constants `LfnAttribute = 0x0F`, `LastEntryFlag = 0x40`, `CharsPerSlot = 13`), sealed `LongFileNameCodec` (`ComputeShortNameChecksum(ReadOnlySpan<byte> 11-byte padded short name)` implementing the canonical rotate-right-then-add algorithm `sum = ((sum & 1) << 7 | sum >> 1) + b`; `EncodeLongName(string, byte checksum)` -> ordered slots topmost-first with NUL terminator + 0xFFFF padding inside the final slot; `WriteSlot(VfatLfnEntry, Span<byte>)` serialises a slot to the 32-byte directory record at offsets 1..10/14..25/28..31 with attribute = 0x0F at offset 11; `TryParseSlot(ReadOnlySpan<byte>)` returns null when attribute != 0x0F; `DecodeLongName(IReadOnlyList<VfatLfnEntry> slotsInDiscOrder, byte expectedChecksum)` validates last-flag on the topmost slot, contiguous descending ordinals, consistent checksum across the chain, and the expected-checksum match, returning null on any mismatch and the reconstructed string up to the first NUL otherwise), sealed `VfatDirectoryRecord` pairing a `MutableFatDirectoryEntry` with an optional decoded long name, and sealed `VfatDirectoryReader.ReadRecords(ReadOnlySpan<byte>)` which walks a raw directory in 32-byte steps, buffers consecutive LFN slots, skips deleted entries (0xE5) by discarding pending slots, skips volume-label entries (attribute bit 0x08 without directory bit 0x10), stops at the end-of-directory marker (0x00), and pairs each accumulated chain with the following 8.3 entry. 14 RED->GREEN tests in `tests/Spice86.Storage.Tests/FileSystem/Directory/LongFileName/LongFileNameCodecTests.cs`: known-vector checksum for "MYFILE  TXT" -> 0xBA, wrong-input-length argument validation, single-slot encode for 9-char names with NUL+0xFFFF padding, two-slot encode for 14-char names topmost-first, no internal NUL for exactly 13 chars, slot byte round-trip through Write+TryParse, non-LFN attribute returns null on TryParse, well-formed chain decode, checksum-mismatch returns null, missing last-flag returns null, reader pairs LFN chain with following short entry, reader yields null long name for standalone 8.3 entries, reader drops slots preceded by a deleted entry, reader stops at end-of-directory marker. Atom B remaining: write integration into `MutableFatDirectoryEntry` and directory writer + 8.3 alias generation. 146/146 storage + 2150/2150 main tests green.
...
- Phase 3 (ISO Joliet & Rock Ridge): Completed (100%) - Atom 1 landed: substantial vertical slice covering 3a + 3b. New `IsoSupplementaryVolumeDescriptor` (sealed) carries the SVD decoded fields plus the 3-byte escape sequence and an `IsJoliet` predicate covering UCS-2 levels 1/2/3 (`0x25 0x2F 0x40/0x43/0x45`). `IsoImage` now scans Volume Descriptors from LBA 16 up to the type-0xFF terminator (32-VD safety bound), validating the CD001 signature each step, and exposes `JolietVolume` (nullable). New `IsoDirectoryRecord.ParseJoliet`/`ParseJolietNullable` decodes record names via `Encoding.BigEndianUnicode` preserving case while stripping `;N` version suffixes and special-casing the `.`/`..` single-byte names. New `IsoImage.ReadJolietRootDirectory()` walks the Joliet root extent sector-by-sector returning fully decoded long filenames; existing `PrimaryVolume` ASCII path remains untouched (DOS consumer in `Dos.cs` unaffected). Test infra: new fluent `Iso9660ImageBuilder` (sealed) synthesises multi-VD ISOs (PVD + optional Joliet SVD + terminator + paired root directories + file extents) and replaces hand-rolled byte arrays. 6 RED-then-GREEN tests in `IsoJolietTests` cover PVD-only (no Joliet exposed), Joliet UCS-2 BE identifier decoding, UCS-2 BE long-filename directory reading, primary path stays ASCII, non-Joliet SVD ignored, and extent-LBA round-trip to file payload. 95/95 storage tests + 2142/2142 main tests green. Atom 2 landed: substantial vertical slice for Phase 3c Rock Ridge SUSP foundation (NM + PX). New `Spice86.Shared.Emulator.Storage.CdRom.RockRidge` namespace adds sealed immutable `SuspEntry` (signature/version/payload, defensive payload copy), sealed `SuspParser.Parse(ReadOnlySpan<byte>)` which iterates SUSP entries from the trailing System Use Area of an ISO 9660 directory record (4-byte header: 2-char ASCII signature + length + version), halting gracefully at malformed lengths, truncated entries, the canonical `ST` terminator, or end-of-buffer; sealed immutable `RockRidgeMetadata` (nullable `AlternateName`, `PosixFileMode`, `FileLinkCount`, `UserId`, `GroupId`, with `HasAny` predicate); sealed `RockRidgeParser.Parse(ReadOnlySpan<byte>)` which scans via `SuspParser` and extracts (a) `NM` alternate name with multi-entry CONTINUE-flag (0x01) concatenation and CURRENT/PARENT-flag (0x02/0x04) suppression for `.`/`..` self/parent entries, plus (b) `PX` POSIX file-mode/links/uid/gid from the 4 BB-encoded (little-endian + big-endian redundant) 8-byte fields, requiring at least 32 payload bytes (`PxOldLength` per RRIP 1.12); the optional `PxWithSerialLength = 40` serial-number field is parked as a constant for future extension. SL (symlinks) / CE (continuation areas) / CL/PL/RE (deep-tree relocation) deferred to follow-up. 12 RED-then-GREEN tests in `tests/Spice86.Storage.Tests/CdRom/RockRidge/RockRidgeParserTests.cs`: empty-buffer enumeration, single SP entry round-trip, ST-terminator stop, malformed-length graceful abort, truncated-entry graceful abort, no-NM-or-PX returns empty metadata with `HasAny == false`, single-NM extraction, two-NM CONTINUE-flag concatenation (`long-` + `filename` -> `long-filename`), NM CURRENT-flag suppression, PX 32-byte BB-encoded extraction of mode/links/uid/gid, PX-too-short leaves all attrs null, NM+PX combined emit name and mode. 132/132 storage + 2150/2150 main tests green.
- Phase 4 (CUE/BIN Audio & Codec Dispatch): Completed (100%) - Atom 1 landed: substantial vertical slice covering 4a + 4b. New `CueFileType` enum (BINARY/MOTOROLA/WAVE/AIFF/MP3/FLAC/OGG/OPUS) and `CueEntry.FileType` property (defaults to `Binary` for legacy CUE sheets that omit the type token). `CueSheetParser` now captures the trailing FILE-directive type token via a dedicated `ParseFileType` regex and threads it onto every emitted entry. New sealed `WavAudioFile` reads RIFF/WAVE PCM headers, walking chunks to locate `fmt ` + `data`, validating CDDA compliance (44100 Hz / 2 ch / 16-bit / PCM format tag 1) and rejecting non-conforming streams with `InvalidDataException`. New sealed `WindowedDataSource` (`IDataSource`) exposes a contiguous byte window into a backing source, used to project a WAV's PCM payload as raw 2352-byte CD audio sectors with zero copy. `CueBinImage` constructor refactored: `_sources` is now `Dictionary<string, IDataSource>` with a parallel `_ownedDisposables` list. Atom 2 landed: substantial vertical slice introducing the full codec-dispatch layer. New `Spice86.Shared.Emulator.Storage.CdRom.Audio` namespace adds `IAudioCodec` (`OpenAsCdda(string) -> IDataSource` returning CDDA-compliant PCM), `IAudioCodecFactory` (`CanHandle(CueFileType, string)` + `Create()`), sealed `CompositeAudioCodecFactory` (chains factories, exposes `CreateFor(fileType, path)` and throws `NotSupportedException` when no factory matches), sealed `WavAudioCodec` (wraps `WavAudioFile` + `FileBackedDataSource` + `WindowedDataSource`, disposable), sealed `WavAudioCodecFactory` (claims `CueFileType.Wave`), sealed `LibVlcAudioCodec` (LibVLCSharp-backed, idempotent `Core.Initialize()` guarded by `Interlocked.CompareExchange`, transcodes any LibVLC-decodable audio to raw `s16l/44100/2ch` PCM via Media `:sout=#transcode{acodec=s16l,channels=2,samplerate=44100}:standard{access=file,mux=raw,dst='<temp>'}`, waits on `EndReached`/`EncounteredError` with a 5-min safety timeout, then exposes the temp file via `FileBackedDataSource`; cleans temp files on Dispose; raises `InvalidOperationException` with native-binary-install hint when `Core.Initialize` fails), sealed `LibVlcAudioCodecFactory` (claims MP3/FLAC/OGG/OPUS/AIFF/MOTOROLA), and static `DefaultAudioCodecFactory.Create()` returning the standard `WavAudioCodecFactory` + `LibVlcAudioCodecFactory` composite. `CueBinImage` gained a second constructor `CueBinImage(string cuePath, CompositeAudioCodecFactory codecFactory)`; the parameterless overload now forwards `DefaultAudioCodecFactory.Create()` (no optional parameters). `CreateSource` now dispatches `Binary → FileBackedDataSource` while every other `CueFileType` is routed through the injected codec factory's `CreateFor`, registering any `IDisposable` codec in `_ownedDisposables` so `Dispose()` correctly tears down LibVLC-owned temp files. Central package management updated: `LibVLCSharp 3.9.4` + `VideoLAN.LibVLC.Windows 3.0.21` (Windows-conditional) added to `src/Directory.Packages.props`, referenced from `Spice86.Storage.Cd.csproj`. 14 new RED-then-GREEN tests in `tests/Spice86.Storage.Tests/CdRom/Audio/`: `AudioCodecFactoryDispatchTests` (Wav factory dispatch, LibVLC factory dispatch across all 6 compressed types, composite first-match-wins ordering, composite NotSupported failure, default factory wiring), `CueBinImageCodecDispatchTests` (NSubstitute-driven MP3 dispatch through injected fake codec with PCM round-trip, codec disposal on `CueBinImage.Dispose()`, NotSupportedException when no codec available), `CueSheetParserAudioFileTypeTests` (theory covering MP3/FLAC/OGG/OPUS/AIFF/MOTOROLA token parsing). 115/115 storage + 2142/2142 main tests green. Atom 3 landed: substantial vertical slice for Phase 4c INDEX 00 in-file pregap layout. New sealed immutable `CueTrackLayout` (per-track resolved geometry: `TrackNumber`, `FileName`, `FileType`, `TrackMode`, `Index01Frames`, nullable `Index00Frames`, `StartLba`, `LengthSectors`, `FileByteOffset`, `SectorSize`, `PregapFrames`, `PostgapFrames`). New sealed `CueFrameMapper.BuildLayout(CueSheet, Func<string,long>)` produces `IReadOnlyList<CueTrackLayout>` from a parsed sheet: groups entries by track number, captures optional INDEX 00 alongside required INDEX 01, applies the Red Book 150-frame offset (`RedbookPreGapFrames` public constant) to derive `StartLba`, and — mirroring DOSBox Staging's `AddTrack` skip semantics — reduces each track's `LengthSectors` by `next.Index01 - next.Index00` whenever the following track declares INDEX 00 (so the previous track's TOC ends precisely at the next track's INDEX 00 boundary). Last-track length is derived from a caller-supplied `fileLengthProvider` delegate (so the mapper stays codec-agnostic). `FileByteOffset = StartLba * SectorSize` preserves byte-position parity with the previous implementation for single-file CUE/BIN images. `CueBinImage.BuildTracks` refactored to delegate end-to-end to `CueFrameMapper`, with a new private `GetFileLengthBytes` callback that reuses already-opened `IDataSource` entries (avoiding double-decode for LibVLC-backed audio tracks) and falls back to `FileInfo.Length` for not-yet-opened BINARY files. The dead `MapSectorSize` helper and the now-redundant `RedbookPreGapFrames` constant were removed from `CueBinImage`. 7 RED-then-GREEN tests in `tests/Spice86.Storage.Tests/CdRom/Layout/CueFrameMapperTests.cs`: two-track-no-INDEX-00 regression, two-track-with-INDEX-00 skip reduction, single-track length from file size, file byte offset arithmetic, pregap/postgap/file metadata preservation, null-sheet guard, null-provider guard. 120/120 storage + 2150/2150 main tests green. Atom 4 landed: substantial vertical slice for Phase 4d Subchannel-Q synthesis. New sealed `SubchannelQData` immutable POCO (attribute, BCD track byte, linear index, relative MSF triplet, absolute MSF triplet — all plain-decimal-not-BCD MSF per DOSBox-staging `MSCDEX_IOCTL_Input` case 0x0C parity). New sealed `SubchannelQSynthesizer.Compute(IReadOnlyList<TableOfContentsEntry>, int currentLba)` performs track lookup with lead-out guard, derives the containing-track attribute and BCD-encoded track number, computes relative MSF from track start (clamped >= 0) and absolute MSF from `lba + 150`. `Mscdex.WriteAudioSubchannel` refactored to instantiate the synthesizer, compute the payload from `drive.GetTableOfContents()` + `drive.GetAudioStatus().CurrentLba`, and emit the 10-byte buffer layout (attr/trkBCD/idx/rel.M/S/F/0/abs.M/S/F) at offsets 1-10. The previously-inlined TOC scan plus the now-orphaned `LeadOutTrackNumber` const were removed from `Mscdex.cs`. 8 RED-then-GREEN tests in `tests/Spice86.Tests/CdRom/Subchannel/SubchannelQSynthesizerTests.cs` covering: start of first track (00:00:00 rel + 00:02:00 abs pregap-aware), 1-second offset, BCD encoding of double-digit tracks (12 -> 0x12), audio vs data attribute selection, relative MSF measured from track start on track 2, lead-out fallback (zero attribute + zero track), frame-level precision at LBA 74, and the always-1 index invariant. Note: cross-platform binaries — Windows is bundled via `VideoLAN.LibVLC.Windows`; Linux/macOS hosts need system `libvlc` to be installed and discoverable.
- Phase 5 (MDS/MDF): Completed (100%) - Substantial vertical slice landed: parser + image + factory dispatch. New `Spice86.Shared.Emulator.Storage.CdRom.Mds` namespace adds sealed enum `MdsTrackMode` (Audio/Mode1Data/Mode2Data), sealed immutable `MdsTrack` (Number/Mode/SectorSize/SubchannelSize/StartSector/SkipBytes/LengthSectors/MdfFilename), sealed immutable `MdsDiscDescriptor` (`IReadOnlyList<MdsTrack> Tracks`), and sealed `MdsParser` mirroring dosbox-staging's `LoadMdsFile` + `cdrom_mds.h` (de-glib'd from cdemu/libmirage): validates 16-byte ASCII signature `"MEDIA DESCRIPTOR"`, rejects unsupported major versions (!= 1), zero sessions, zero session-block offset, zero track-block offset, multi-file tracks, sector-size <= subchannel-size, non-contiguous track numbers (point must equal previous+1), and overlapping start sectors; skips non-track entries (point < 1 or > 99 — covers lead-in 0xA0 / lead-out 0xA2 markers); decodes modes via `mode & 0x07` (1→Audio, 2→Mode1Data, 0/3/7→Mode2Data, 4/5/6→reject); decodes subchannel raw byte (0→0, 8→96, other→0); reads MDF filenames as NUL-terminated ASCII when widechar flag == 0 else NUL-terminated UTF-16 LE. New sealed `MdsImage : ICdRomImage` (sits next to `IsoImage`/`CueBinImage`, not under the `Mds/` sub-namespace): parses the MDS descriptor, opens the referenced MDF data files (resolves `*.mdf` placeholder relative to `<mdsBasename>.mdf`, accepts rooted paths as-is, otherwise resolves relative to the MDS directory), shares one `FileBackedDataSource` per unique MDF path (case-insensitive dictionary), builds `CdTrack` entries via `MapMode` (Audio→AudioRaw2352; Mode1Data→CookedData2048 if sectorSize==2048 else Raw2352; Mode2Data→Mode2Form1), appends a synthetic lead-out (number=0, length=0, startLba = lastStart + lastLength), parses the PVD from the first data track using `SectorFraming.ExtractCookedFromRaw2352`/`ExtractMode2Form1FromRaw2352` when the on-disk sector is raw, and on `Read` skips the lead-out and routes cooked extraction through `SectorFraming`. `CdRomImageFactory.Open` now dispatches `.mds` to `MdsImage` alongside `.iso`/`.cue`; error message and XML docs updated. New test infrastructure: sealed fluent `MdsImageBuilder` (`tests/Spice86.Storage.Tests/CdRom/Mds/MdsImageBuilder.cs`) synthesises a valid MDS+MDF pair (header at 0..88, session block at 88..112 with `numAllBlocks`/`numTrackBlocks`/first-track/last-track/track-block-offset fields, N×80-byte track blocks at 112+, per-track 8-byte extra blocks + 16-byte footers + NUL-terminated ASCII/UTF-16 filenames; the paired MDF lays down a minimally-valid ISO 9660 PVD at the first data track's LBA 16 so the PVD parser succeeds). 12 RED-then-GREEN tests: parser tests (single data track, two-tracks data+audio, non-track-block skip, invalid-signature reject, unsupported-version reject, zero-sessions reject, wide-char filename decode), image tests (synthetic lead-out append, multi-track LBA boundaries, PVD-like sector readback via cooked extraction, factory dispatch via `CdRomImageFactory.Open(*.mds)`, unknown-extension throw). 145/145 storage + 2150/2150 main tests green.
...
| MDS/MDF (Alcohol 120%) image format | `cdrom_image.cpp` `LoadMdsFile` + `cdrom_mds.h` | Done (Phase 5). |
...
| Status | **COMPLETED** |
| Progress | 100% |
| Atoms shipped | Single substantial vertical slice: `MdsTrackMode` enum + `MdsTrack` POCO + `MdsDiscDescriptor` + `MdsParser` (full Alcohol 120% descriptor parser at dosbox-staging parity) + `MdsImage : ICdRomImage` (file resolution, multi-track CdTrack build, synthetic lead-out, PVD parse, sector read with cooked extraction) + `.mds` factory dispatch in `CdRomImageFactory.Open`. |
| dosbox-staging parity | **Parity achieved.** Mirrors `cdrom_image.cpp::LoadMdsFile` + `cdrom_mds.h` (de-glib'd from cdemu/libmirage). Header validation, session-block walk, track-block iteration, non-track-point skipping (0xA0/0xA2), mode + subchannel decode, ASCII/UTF-16 filename reading, `*.mdf` placeholder resolution, contiguous-track invariant. |
| Tests added | 12 RED-then-GREEN tests in `tests/Spice86.Storage.Tests/CdRom/Mds/` (7 parser + 5 image/factory) using a new sealed fluent `MdsImageBuilder`. 145/145 storage + 2150/2150 main tests green. |
| Remaining work | None. |
