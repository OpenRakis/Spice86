...
- **NO SHORTCUTS OR STUBS** - **NEVER** simplify implementations, use in-memory collections as stubs, skip real implementation, or create MVP versions. Always implement the full, correct algorithm. This applies to file I/O, cluster allocation, FAT chains, directory entries, memory management, or any subsystem. Every method must solve the actual problem, not a simplified version. Violating this rule will cause technical debt and cascading failures.
- **NO SHORTCUTS IN TESTS** - tests must use the project's existing fluent image builders (e.g. `Fat12ImageBuilder`) and real serialization paths. Do NOT hand-roll minimal byte arrays as a substitute for a real disk image, and do NOT bypass real serialization just to make a test pass.
- **NEVER fabricate environment issues** - do NOT blame build cache, Avalonia DLL locks, or "environment problems" to justify skipping verification. If `dotnet build` or `dotnet test` fails, the implementation has a real bug - diagnose and fix it. Builds and tests are NEVER skipped.
